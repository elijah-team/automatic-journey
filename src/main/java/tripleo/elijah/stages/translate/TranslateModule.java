...
public class TranslateModule {
    ...
    private static TabbedOutputStream stream_for(final String packageName, final String name)
        ...
        if (packageDirName.equals("")) packageDirName = ".";
    ...
    public void translate() {
        ...
        for (final ModuleItem item : module.getItems()) {
            try {
                ...
                } else SimplePrintLoggerToRemoveSoon.println2("8000 " + item);
    ...
    private void put_class_statement_internal(final ClassStatement classStatement, final TabbedOutputStream w)
        ...
        for (final ClassItem item : classStatement.getItems()) {
            if (item instanceof FunctionDef) {
                ...
                w.dec_tabs();
                w.put_string_ln("}");
            } else SimplePrintLoggerToRemoveSoon.println2("8001 " + item);
        }
    }

    private void put_namespace_statement_internal(
            final NamespaceStatement namespaceStatement, final TabbedOutputStream w) {
