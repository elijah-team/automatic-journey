...
public class GenerateResultItem {
    ...
    public GenerateResultItem(
            final @NotNull GenerateResult.TY aTy,
    ...
    public List<DependencyRef> dependencies() {
        //		List<DependencyRef> x = Lists.transform(new ArrayList<>(dependency.deps), new Function<Dependency,
        // DependencyRef>() {
        //			@Override
        ...
        //			}
        //		});
        final List<DependencyRef> x =
                dependency.getNotedDeps().stream().map(dep -> dep.dref).collect(Collectors.toList());
        return x;
    }
}
