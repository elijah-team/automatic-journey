...
class CRI_Ident {
    ...
    public String getIdentIAPath(
        ...
        if (resolved_element != null) {
            ...
            if (resolved_element instanceof ClassStatement) {
                ...
                resolved = _re_is_PropertyStatement(
                        addRef, aog, sSize, i, aValue, (x) -> skip[0] = true, (x) -> text[0] = x);
            }
            if (!skip[0]) {
                short state = 1;
                ...
                switch (state) {
                    case 1:
                        ...
                        if (sSize >= i + 1) {
                            ...
                            final boolean b = _getIdentIAPath_IdentIAHelper(
                                    s.get(i + 1),
                                    sl,
                                    i,
                                    sSize,
            ...
            switch (ite.getStatus()) {
                ...
                case UNKNOWN:
                    ...
                    if (sl.size() == 0) {
                        text[0] =
                                Emit.emit("/*149*/") + text1; // TODO check if it belongs somewhere else (what does this
                        // mean?)
                    } else {
    ...
    private GeneratedNode _re_is_FunctionDef(final @Nullable ProcTableEntry pte, final GeneratedClass a_cheat) {
        ...
        if (resolved == null) {
            ...
            if (resolved1 instanceof GeneratedFunction) resolved = resolved1;
    ...
    private GeneratedNode _re_is_PropertyStatement(
            final Consumer<CReference.Reference> addRef,
            final Generate_Code_For_Method.AOG aog,
            final int sSize,
            final int i,
        ...
        final int code;
        if (resolved1 != null) code = ((GeneratedContainerNC) resolved1).getCode();
        else code = -1;
        short state = 0;
        if (i < sSize - 1) {
            state = 1;
        ...
        switch (state) {
            case 1:
                addRef.accept(new CReference.Reference(
                        String.format("ZP%d_get%s(", code, ite.getIdent().getText()), CReference.Ref.PROPERTY_GET));
                skip.accept(null);
                text.accept(null);
                break;
            case 2:
                addRef.accept(new CReference.Reference(
                        String.format("ZP%d_set%s(", code, ite.getIdent().getText()),
                        CReference.Ref.PROPERTY_SET,
                        aValue));
                skip.accept(null);
                text.accept(null);
                break;
    ...
    boolean _getIdentIAPath_IdentIAHelper(
            final InstructionArgument ia_next,
            final List<String> sl,
            final int i,
            final int sSize,
            ...
            final String aValue,
            final CReference aCReference) {
        return new CReference_getIdentIAPath_IdentIAHelper(
                        ia_next, sl, i, sSize, resolved_element, generatedFunction, aResolved, aValue)
                .action(this, aCReference);
    }
}
