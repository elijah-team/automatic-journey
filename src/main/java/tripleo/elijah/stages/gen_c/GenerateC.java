...
//			if (generatedNode.module() != mod) continue; // README curious
	...
	private void generateCodeForMethod(final BaseGeneratedFunction gf, final GenerateResult gr,
...
//	}
	...
	static class GetAssignmentValue {
		...
		public String FnCallArgs(final @NotNull FnCallArgs fca, final @NotNull BaseGeneratedFunction gf,
				final ElLog LOG) {
...
//					assert false; // TODO synthetic methods
		...
		@NotNull
		private List<String> getAssignmentValueArgs(final @NotNull Instruction inst, final BaseGeneratedFunction gf,
				final ElLog LOG) {
...
//			LOG.err(("9001-3 "+cte.initialValue));
		...
		public String forClassInvocation(final @NotNull Instruction aInstruction, final ClassInvocation aClsinv,
				final @NotNull BaseGeneratedFunction gf, final ElLog LOG) {
			final int y = 2;
			...
			@NotNull
			final List<String> x = getAssignmentValueArgs(aInstruction, gf, LOG);
			reference.args(x);
