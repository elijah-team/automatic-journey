...
class Found_Element_For_ITE {
    ...
    public void action(@NotNull final IdentTableEntry ite) {
        ...
        final String normal_path =
                generatedFunction.getIdentIAPathNormal(new IdentIA(ite.getIndex(), generatedFunction));
        if (!ite.resolveExpectation.isSatisfied()) ite.resolveExpectation.satisfy(normal_path);
    }
            ...
            if (!(typeName.isNull())) {
                ...
                if (parent instanceof NamespaceStatement || parent instanceof ClassStatement) {
                    ...
                    if (generatedFunction instanceof final @NotNull GeneratedFunction generatedFunction1) {
                        ...
                        state = (parent
                                != ((GeneratedConstructor) generatedFunction)
                                        .getFD()
                                        .getParent());
                    }
                    if (state) {
                        ...
                        dm.typePromise().done(new DoneCallback<GenType>() {
                            ...
                            public void onDone(@NotNull final GenType result) {
                                ...
                                final String normal_path = generatedFunction.getIdentIAPathNormal(
                                        new IdentIA(ite.getIndex(), generatedFunction));
                        ...
                        dm.typePromise().then(new DoneCallback<GenType>() {
                            ...
                            public void onDone(final GenType result) {
                                ...
                                assert result.getResolved() != null;
                                ite.setGenType(result);
                                //								ite.resolveTypeToClass(result.node); // TODO setting this has no effect on
                                // output
                    ...
                    if (parent instanceof NamespaceStatement) genType = new GenType((NamespaceStatement) parent);
                    else if (parent instanceof ClassStatement) genType = new GenType((ClassStatement) parent);
    ...
    public void action_ClassStatement(@NotNull final IdentTableEntry ite, final ClassStatement classStatement) {
        ...
        } else ite.type.setAttached(attached);
    }
    ...
    public void action_PropertyStatement(@NotNull final IdentTableEntry ite, @NotNull final PropertyStatement ps) {
        ...
        switch (ps.getTypeName().kindOfType()) {
            ...
            case NORMAL:
                try {
                    attached = (dc.resolve_type(new OS_UserType(ps.getTypeName()), ctx)
                                    .getResolved()
                                    .getClassOf())
                            .getOS_Type();
            ...
            default:
                throw new IllegalStateException(
                        "Unexpected value: " + ps.getTypeName().kindOfType());
        }
        if (ite.type == null) {
            ite.makeType(generatedFunction, TypeTableEntry.Type.TRANSIENT, attached);
        } else ite.type.setAttached(attached);
