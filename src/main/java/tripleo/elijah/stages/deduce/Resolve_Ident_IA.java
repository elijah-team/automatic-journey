...
public class Resolve_Ident_IA {
    ...
    @Contract(pure = true)
    public Resolve_Ident_IA(
            final @NotNull DeduceTypes2.DeduceClient3 aDeduceClient3,
    ...
    public void action() throws ResolveError {
        ...
        /*
         * for (final InstructionArgument ia : s) { if (ia instanceof IntegerIA) {
         *
         * @NotNull RIA_STATE state = action_IntegerIA(ia); switch (state) { case
         * CONTINUE: continue; case RETURN: return; case NEXT: break; default: throw new
         * IllegalStateException("Can't be here"); } } else if (ia instanceof IdentIA) {
         *
         * @NotNull RIA_STATE state = action_IdentIA((IdentIA) ia); switch (state) {
         * case CONTINUE: continue; // never happens here case RETURN: return; //
         * element notFound. short-circuit and exit. callback already called. case NEXT:
         * break; default: throw new IllegalStateException("Can't be here"); } } else if
         * (ia instanceof ProcIA) { action_ProcIA(ia); } else throw new
         * IllegalStateException("Really cant be here");
         */
    ...
    private boolean process(final InstructionArgument ia, final @NotNull List<InstructionArgument> aS)
            throws ResolveError {
        if (ia instanceof IntegerIA) {
            ...
            action_ProcIA(ia);
        } else throw new IllegalStateException("Really cant be here");
        return true;
    }
    ...
    private @NotNull RIA_STATE action_IdentIA(@NotNull final IdentIA ia) {
        final @NotNull __Action_IdentIA action_identIA =
                new __Action_IdentIA(this, ia, foundElement, phase, generatedFunction, dc, LOG, identIA);
    ...
    private void action_ProcIA(@NotNull final InstructionArgument ia) throws ResolveError {
        ...
        if (prte.getResolvedElement() == null) {
            ...
            if (exp instanceof final @NotNull ProcedureCallExpression pce) {
                ...
                if (exp instanceof ProcedureCallExpression) throw new IllegalStateException("double pce!");
            } else throw new IllegalStateException("prte resolvedElement not ProcCallExpression");
        ...
        switch (aAttached.getType()) {
            ...
            case USER:
                if (el instanceof MatchConditional.MatchArm_TypeMatch) {
                    ...
                    //						ectx = el.getContext();
                } else ectx = aAttached.getTypeName().getContext(); // TODO is this right?
                break;
            case FUNC_EXPR: {
    ...
    private void action_002(final @NotNull TypeTableEntry tte) {
        ...
        if (tte.expression instanceof ProcedureCallExpression) {
            if (tte.tableEntry != null) {
                ...
                } else throw new IllegalStateException("tableEntry must be ProcTableEntry");
            }
        }
    }

    private void _procIA_constructor_helper(@NotNull final ProcTableEntry pte) {
        if (pte.getClassInvocation() != null) throw new IllegalStateException();
        ...
        if (pte.getFunctionInvocation() == null) {
            ...
            if (fi.getFunction() instanceof ConstructorDef) {
                ...
                else j = new WlGenerateCtor(generateFunctions, fi, null, phase.getCodeRegistrar());
                dc.addJobs(j);
    ...
    private void action_002_no_resolved_element(final _action_002_no_resolved_element action) {
        ...
        if (_backlink instanceof ProcIA) {
            ...
            final DeduceElement3_VariableTableEntry vte_de3 =
                    (DeduceElement3_VariableTableEntry) backlink.getDeduceElement3();
    ...
    private void action_002_1(
            @NotNull final ProcTableEntry pte, @NotNull final IdentTableEntry ite, final boolean setClassInvocation) {
    ...
    public record _action_002_no_resolved_element(
            IdentIA identIA, IdentTableEntry ite, ProcTableEntry pte, Resolve_Ident_IA ria) {
    ...
    static class __Action_IdentIA {
            ...
            if (idte.getStatus() == BaseTableEntry.Status.UNCHECKED) {
                if (idte.getBacklink() == null) {
                    ...
                    if (el != null) {
                        ...
                        if (el.getContext() == null) throw new IllegalStateException("2468 null context");
        ...
        public boolean run_one() {
            if (idte.getStatus() == BaseTableEntry.Status.UNKNOWN) {
                ...
                if (idte.resolveExpectation != null) idte.resolveExpectation.fail();
                foundElement.doNoFoundElement();
                return true;
            }
        ...
        private void __el_is_FunctionDef() {
            ...
            if (genType != null) {
                ...
                if (invocation != null) {
                    ...
                    final FunctionInvocation fi =
                            new FunctionInvocation((BaseFunctionDef) el, null, invocation, phase.getGeneratePhase());
            ...
            final @NotNull FunctionInvocation fi =
                    dc.newFunctionInvocation((BaseFunctionDef) el, callablePTE, invocation);
        ...
        private void __backlink_is_not_null() {
            dc.resolveIdentIA2_(
                    ectx /* context */, ia, null, generatedFunction, new FoundElement(phase) {
