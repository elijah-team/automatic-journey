...
public class ProcTableListener implements BaseTableEntry.StatusListener {
    ...
    public void onChange(final IElementHolder eh, final BaseTableEntry.Status newStatus) {
        ...
        if (newStatus != BaseTableEntry.Status.UNKNOWN) { // means eh is null
            ...
            } else depTracker = null;

            set_resolved_element_pte(co, eh.getElement(), pte, depTracker);
        }
    }

    void set_resolved_element_pte(
            final @Nullable Constructable co,
            final OS_Element e,
    ...
    private void resolved_element_pte_FunctionDef(
            final Constructable co,
        ...
        if (pte.expression_num != null) {
            ...
            if (dp.size() > 1) {
                ...
                if (parent instanceof IdentExpression) {
                    ...
                    resolved_element_pte_FunctionDef_FormalArgListItem(
                            co, pte, depTracker, fd, (FormalArgListItem) parent);
                } else if (parent instanceof VariableStatement) {
                    resolved_element_pte_FunctionDef_VariableStatement(
                            co, pte, depTracker, fd, (VariableStatement) parent);
                } else {
                    @NotNull
    ...
    private void resolved_element_pte_FunctionDef_IdentExpression(
            final Constructable co,
            final ProcTableEntry pte,
            final AbstractDependencyTracker depTracker,
            @NotNull final FunctionDef fd,
    ...
    private void resolved_element_pte_FunctionDef_FormalArgListItem(
            final Constructable co,
            final ProcTableEntry pte,
            final AbstractDependencyTracker depTracker,
            @NotNull final FunctionDef fd,
            final FormalArgListItem parent) {
    ...
    private void resolved_element_pte_FunctionDef_VariableStatement(
            final Constructable aCo,
    ...
    void finish(
            @Nullable final Constructable co,
            @Nullable final AbstractDependencyTracker depTracker,
        ...
        if (depTracker != null) {
            if (aGenType == null) SimplePrintLoggerToRemoveSoon.println_err2("247 genType is null");

            if (
            /* aGenType == null && */ aFi.getFunction() instanceof ConstructorDef) {
    ...
    private void resolved_element_pte_FunctionDef_dunder(
            final Constructable co,
            final AbstractDependencyTracker depTracker,
            @NotNull final ProcTableEntry pte,
            @NotNull final FunctionDef fd,
            ...
            if (fd == parent) {
                ...
                if (x.getAttached() == null && x.tableEntry == null) {
                    ...
                    if (vte_ia != null) {
                        ...
                        typeName = gt.getNonGenericTypeName() != null
                                ? gt.getNonGenericTypeName()
        ...
        if (ia != null) {
            if (ia instanceof IdentIA) {
                ...
                final ClassInvocation ci =
                        procTableEntry.getFunctionInvocation().getClassInvocation();
                if (ci != null) {
                    VTE_TypePromises.resolved_element_pte_VariableStatement(
                            co, depTracker, fd, variableStatement, procTableEntry, ci, this);
                } else {
                    assert false;
                }
            } else {
    ...
    class E_Is_FunctionDef {
        ...
        public @NotNull E_Is_FunctionDef invoke(final TypeName typeName) {
            if (pte.getClassInvocation() == null && pte.getFunctionInvocation() == null) {
                ...
                } else throw new IllegalStateException("Unknown parent");
                if (fi != null) pte.setFunctionInvocation(fi);
