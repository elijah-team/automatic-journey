...
public class ResolveType {
    static @NotNull GenType resolve_type(
        ...
        if (type.getType() != OS_Type.Type.USER_CLASS) R.setTypeName(type);
    ...
    private static void resolve_built_in(
            final OS_Module module, final @NotNull OS_Type type, final DeduceTypes2 dt2, final @NotNull GenType aR)
            throws ResolveError {
    ...
    private static void resolve_user(
            final @NotNull OS_Type type, final ElLog LOG, final DeduceTypes2 dt2, final @NotNull GenType aR)
            throws ResolveError {
        ...
        switch (tn1.kindOfType()) {
            case NORMAL: {
                ...
                } else aR.setResolved(((ClassStatement) best).getOS_Type());
                break;
            }
            case FUNCTION:
            case GENERIC:
            ...
            case TYPE_OF:
                {
                    ...
                    if (q.parts().size() == 1 && q.parts().get(0).getText().equals("self")) {
                        ...
                        aR.setResolved(((ClassContext) type_of.getContext())
                                .getCarrier()
                                .getOS_Type());
                    }
                    final int y = 2;
