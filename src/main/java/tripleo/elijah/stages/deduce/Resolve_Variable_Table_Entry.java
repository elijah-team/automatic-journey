...
class Resolve_Variable_Table_Entry {
	...
	public Resolve_Variable_Table_Entry(final BaseGeneratedFunction aGeneratedFunction, final Context aCtx,
	...
	public void action_VAR_potsize_1_other(@NotNull final VariableTableEntry vte, @NotNull final TypeTableEntry aPot) {
		try {
			if (aPot.tableEntry instanceof final @NotNull ProcTableEntry pte1) {
				@Nullable
				final OS_Element e = DeduceLookupUtils.lookup(pte1.expression, ctx, deduceTypes2);
				if (e == null) {
...
//						final FunctionDef fd = (FunctionDef) e;
					@NotNull
					final IdentTableEntry ite1 = ((IdentIA) pte1.expression_num).getEntry();
					final DeducePath dp = ite1.buildDeducePath(generatedFunction);
					@Nullable
					final GenType t = dp.getType(dp.size() - 1);
...
//			fi = newFunctionInvocation(fd, pte, ci, phase);
	...
	public void action_VAR_potsize_1_and_FuncExprType(@NotNull final VariableTableEntry vte,
			@NotNull final OS_FuncExprType funcExprType, @NotNull final GenType aGenType,
		...
		// add namespace
		@NotNull
		final OS_Module mod1 = fe.getContext().module();
		@Nullable
		final NamespaceStatement mod_ns = lookup_module_namespace(mod1);
		...
		if (mod_ns != null) {
			...
			@NotNull
			final FunctionDef fd1 = new FunctionDef(mod_ns, mod_ns.getContext());
			fd1.setFal(fe.fal());
...
//			tripleo.elijah.util.Stupidity.println_out("1630 "+mod_ns.getItems()); // element 0 is ctor$0
			...
			final DeduceTypes2.@NotNull PromiseExpectation<GenType> pe = deduceTypes2
	...
	private @Nullable ProcTableEntry findProcTableEntry(@NotNull final BaseGeneratedFunction aGeneratedFunction,
			final IExpression aExpression) {
		for (@NotNull
		final ProcTableEntry procTableEntry : aGeneratedFunction.prte_list) {
	...
	public @Nullable NamespaceStatement lookup_module_namespace(@NotNull final OS_Module aModule) {
		try {
			...
			@Nullable
			final OS_Element e = DeduceLookupUtils.lookup(module_ident, aModule.getContext(), deduceTypes2);
			if (e != null) {
				...
				@NotNull
				final NamespaceStatement ns = new NamespaceStatement(aModule, aModule.getContext());
