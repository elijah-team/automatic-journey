...
public class VTE_TypePromises {
    ...
    static void resolved_element_pte(
        ...
        aVariableTableEntry.typePromise().then(new DoneCallback<GenType>() {
            ...
            public void onDone(@NotNull final GenType result) {
                ...
                @NotNull
                final ProcTableListener.E_Is_FunctionDef e_Is_FunctionDef = aProcTableListener
                        .new E_Is_FunctionDef(pte, fd, fd.getParent())
                        .invoke(aVariableTableEntry.type.genType.getNonGenericTypeName());
    ...
    static void resolved_element_pte_VariableStatement(
            final Constructable co,
            final AbstractDependencyTracker depTracker,
            final @NotNull FunctionDef fd,
            ...
            public void onDone(final GeneratedClass result) {
                for (final GeneratedContainer.VarTableEntry varTableEntry : result.varTable) {
                    if (varTableEntry.nameToken.getText().equals(variableStatement.getName())) {
                        ...
                        @NotNull
                        final ProcTableListener.E_Is_FunctionDef e_Is_FunctionDef = aProcTableListener
                                .new E_Is_FunctionDef(aProcTableEntry, fd, fd.getParent())
    ...
    static void resolved_element_pte_VariableStatement2(
            final Constructable co,
            final AbstractDependencyTracker depTracker,
            final ProcTableEntry pte,
            final @NotNull FunctionDef fd,
                    ...
                    if (best2 != null) {
                        if (best2 instanceof @Nullable final FunctionDef fd) {
                            ...
                            final IInvocation invocation =
                                    aDeduceTypes2.getInvocation((GeneratedFunction) generatedFunction);
    ...
    static Promise<GenType, Void, Void> do_assign_call_args_ident_vte_promise(
    ...
    static void dunder(
            final String pn, final IntegerIA aIntegerIA, final ProcTableEntry pte, final DeduceTypes2 aDeduceTypes2) {
        aIntegerIA.getEntry().typePromise().then(new DoneCallback<GenType>() {
            @Override
            public void onDone(@NotNull final GenType result) {
                final boolean found1 = aDeduceTypes2.lookup_name_calls(
                        result.getResolved().getClassOf().getContext(), pn, pte);
                if (found1) {
                    final int y = 2;
                    ...
                    } else SimplePrintLoggerToRemoveSoon.println2("175 pte.fi is not null");
    ...
    static void found_parent(
            final @NotNull DeduceTypes2.PromiseExpectation<GenType> aPromiseExpectation,
                    ...
                    switch (attached1.getType()) {
                        case USER_CLASS:
                            if (ite.type.getAttached() == null)
                                ...
                                aDeduceTypes2.LOG.err(String.format(
                                        "3603 Trying to set %s to %s", ite.type.getAttached(), attached1));
                            }
                            break;
                        case USER:
                            try {
                                @NotNull
                                final GenType ty3 = aDeduceTypes2.resolve_type(
