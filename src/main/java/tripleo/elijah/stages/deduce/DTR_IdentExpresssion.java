...
public class DTR_IdentExpresssion {
	...
	public DTR_IdentExpresssion(final DeduceTypeResolve aDeduceTypeResolve, final IdentExpression aIdentExpression,
	...
	public void run(final IElementHolder eh, final GenType genType) {
		if (eh instanceof final GenericElementHolderWithIntegerIA eh1) {
			...
			variableTableEntry.typeResolvePromise().then(new DoneCallback<GenType>() {
				@Override
