...
public class DeducePhase {
	...
	private final          Map<NamespaceStatement, NamespaceInvocation> namespaceInvocationMap  = new HashMap<NamespaceStatement, NamespaceInvocation>();
	...
	@NotNull
	private                List<FunctionMapHook>                        functionMapHooks        = new ArrayList<FunctionMapHook>();
	@NotNull
	private
	...
	public DeducePhase(final GeneratePhase aGeneratePhase, final PipelineLogic aPipelineLogic,
	                   final ElLog.Verbosity verbosity, final Compilation aCompilation) {
	...
	public void registerResolvedVariable(final IdentTableEntry identTableEntry, final OS_Element parent,
	...
	public @Nullable ClassInvocation registerClassInvocation(@NotNull final ClassInvocation aClassInvocation) {
		...
		wl.addJob(
				new WlGenerateClass(generatePhase.getGenerateFunctions(mod), Result, generatedClasses, codeRegistrar)); // TODO
		// why
		// add
		// now?
...
//					return aClassInvocation;
		...
		for (final GeneratedNode lgci : generatedClasses) {
			if (lgci instanceof GeneratedClass) {
				final List<GeneratedFunction> generatedFunctions = (((GeneratedClass) lgci).functionMap.values())
						.stream().map((final GeneratedFunction generatedFunction) -> {
	...
	public @NotNull DeduceTypes2 deduceModule(@NotNull final OS_Module m, @NotNull final Iterable<GeneratedNode> lgf,
...
//		assert matching_class_list.size() == generatedClasses.size();
	 ...
	 * @param verbosity
	 */
	public void deduceModule(@NotNull final OS_Module m, @NotNull final Iterable<GeneratedNode> lgc,
	                         final boolean _unused, final ElLog.Verbosity verbosity) {
		final @NotNull List<GeneratedNode> lgf = new ArrayList<GeneratedNode>();
		...
		for (@NotNull final GeneratedNode lgci : lgc) {
			...
			if (lgci instanceof GeneratedClass) {
				final @NotNull Collection<GeneratedFunction> generatedFunctions = ((GeneratedClass) lgci).functionMap
						.values();
...
//					generatedFunction.setClass(lgci); // TODO delete when done
				...
				final @NotNull Collection<GeneratedFunction> generatedFunctions = ((GeneratedNamespace) lgci).functionMap
						.values();
				for (@NotNull final GeneratedFunction generatedFunction : generatedFunctions) {
					generatedFunction.setClass(lgci);
				}
	...
	public void forFunction(final DeduceTypes2 deduceTypes2, @NotNull final FunctionInvocation fi,
		...
		for (final GeneratedNode generatedNode : lgc22) {
			if (generatedNode instanceof final @NotNull GeneratedContainer generatedContainer) {
				...
				for (@NotNull final ResolvedVariables resolvedVariables : x) {
					final GeneratedContainer.VarTableEntry variable = generatedContainer
			...
			if (deferredMember.getParent() instanceof final @NotNull NamespaceStatement parent) {
				...
				nsi.resolveDeferred().done(new DoneCallback<GeneratedNamespace>() {
					...
					public void onDone(@NotNull final GeneratedNamespace result) {
						final GeneratedContainer.@Nullable VarTableEntry v = result
...
//								}
		...
		sanityChecks();
		for (final Map.@NotNull Entry<FunctionDef, Collection<GeneratedFunction>> entry : functionMap.asMap()
				.entrySet()) {
...
//	Map<GeneratedFunction, OS_Type> typeDecideds = new HashMap<GeneratedFunction, OS_Type>();
	...
	public @Nullable ClassInvocation registerClassInvocation(final ClassStatement aParent, final String aO) {
		@Nullable
		ClassInvocation ci = new ClassInvocation(aParent, aO);
		ci = registerClassInvocation(ci);
		return ci;
	}
...
//				sanityChecks(generatedNamespace.constructors.values());
	...
	public Map<NamespaceStatement, NamespaceInvocation> getNamespaceInvocationMap() {
		return namespaceInvocationMap;
	}
	...
	public void setGeneratedClasses(@NotNull GeneratedClasses generatedClasses) {
	...
	public void setFunctionMapHooks(@NotNull List<FunctionMapHook> functionMapHooks) {
		this.functionMapHooks = functionMapHooks;
	}
	...
	public void setResolved_variables(@NotNull Multimap<OS_Element, ResolvedVariables> resolved_variables) {
		this.resolved_variables = resolved_variables;
	}

	public @NotNull Multimap<ClassStatement, OnClass> getOnclasses() {
		return onclasses;
	}

	public void setOnclasses(@NotNull Multimap<ClassStatement, OnClass> onclasses) {
		this.onclasses = onclasses;
	}

	public @NotNull Multimap<ClassStatement, ClassInvocation> getClassInvocationMultimap() {
		return classInvocationMultimap;
	}

	public void setClassInvocationMultimap(@NotNull Multimap<ClassStatement, ClassInvocation> classInvocationMultimap) {
		this.classInvocationMultimap = classInvocationMultimap;
	}

	public @NotNull List<DeferredMember> getDeferredMembers() {
		return deferredMembers;
	}

	public void setDeferredMembers(@NotNull List<DeferredMember> deferredMembers) {
		this.deferredMembers = deferredMembers;
	}
	...
	static class ResolvedVariables {
		...
		public ResolvedVariables(final IdentTableEntry aIdentTableEntry, final OS_Element aParent,
	...
	public static class GeneratedClasses implements Iterable<GeneratedNode> {
		...
		public List<GeneratedClass> filterClasses(final Predicate<GeneratedClass> pgc) {
			return generatedClasses.stream().filter(x -> {
				if (x instanceof GeneratedClass) {
					return pgc.test((GeneratedClass) x);
				} else {
					return false;
				}
			}).map(x -> (GeneratedClass) x).collect(Collectors.toList());
		}
	}
}
