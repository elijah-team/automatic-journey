...
public class DeducePhase {
    ...
    private final Map<NamespaceStatement, NamespaceInvocation> namespaceInvocationMap =
            new HashMap<NamespaceStatement, NamespaceInvocation>();
    ...
    @NotNull
    private Multimap<OS_Element, ResolvedVariables> resolved_variables = ArrayListMultimap.create();

    @NotNull
    private Multimap<ClassStatement, OnClass> onclasses = ArrayListMultimap.create();
    // Multimap<GeneratedClass, ClassInvocation> generatedClasses1 =
    // ArrayListMultimap.create();
    @NotNull
    private Multimap<ClassStatement, ClassInvocation> classInvocationMultimap = ArrayListMultimap.create();

    @NotNull
    private List<DeferredMember> deferredMembers = new ArrayList<DeferredMember>();
    ...
    public @Nullable ClassInvocation registerClassInvocation(@NotNull final ClassInvocation aClassInvocation) {
        ...
        for (@NotNull final ClassInvocation ci : cis) {
            ...
            if (ci.getConstructorName() != null)
                if (!(ci.getConstructorName().equals(aClassInvocation.getConstructorName()))) continue;
        ...
        wl.addJob(new WlGenerateClass(
                generatePhase.getGenerateFunctions(mod), Result, generatedClasses, codeRegistrar)); // TODO
        // why
        // add
        // now?
    ...
    public boolean equivalentGenericPart(@NotNull final ClassInvocation first, @NotNull final ClassInvocation second) {
        ...
        for (final Map.@NotNull Entry<TypeName, OS_Type> entry : secondGenericPart.entrySet()) {
            ...
            if (entry_type.equals(entry.getValue())) i--;
            //				else
    ...
    public @NotNull Promise<ClassDefinition, Diagnostic, Void> generateClass(
    ...
    public void finish(@NotNull final GeneratedClasses lgc22) {
        ...
        for (final Map.@NotNull Entry<IdentTableEntry, OnType> entry : idte_type_callbacks.entrySet()) {
            ...
            if (idte.type != null
                    ...
                    idte.type.getAttached() != null) entry.getValue().typeDeduced(idte.type.getAttached());
            else entry.getValue().noTypeFound();
        }
        /*
        ...
        for (final GeneratedNode generatedNode : lgc22) {
            if (generatedNode instanceof final @NotNull GeneratedContainer generatedContainer) {
                ...
                for (@NotNull final ResolvedVariables resolvedVariables : x) {
                    ...
                    if (type != null) variable.addPotentialTypes(List_of(type));
