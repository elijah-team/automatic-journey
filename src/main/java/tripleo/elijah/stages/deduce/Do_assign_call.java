            ...
            if (vte.getResolvedElement() != null) {
                try {
                    ...
                    if (vte.getResolvedElement() instanceof IdentExpression)
                        ...
                        el = DeduceLookupUtils.lookup(
                                ((VariableStatement) vte.getResolvedElement()).getNameToken(), ctx, deduceTypes2);
                            ...
                            result.generateDeferred().done(new DoneCallback<BaseGeneratedFunction>() {
                                ...
                                public void onDone(@NotNull final BaseGeneratedFunction bgf) {
                                    ...
                                    final DeduceTypes2.PromiseExpectation<GenType> pe =
                                            deduceTypes2.promiseExpectation(bgf, "Function Result type");
        ...
        for (int i = 0; i < args.size(); i++) {
            ...
            switch (e.getKind()) {
                ...
                case CHAR_LITERAL:
                    ...
                    break;
                case IDENT:
                    deduceTypes2.do_assign_call_args_ident(
                ...
                case PROCEDURE_CALL:
                    {
                        ...
                        try {
                            final LookupResultList lrl =
                                    DeduceLookupUtils.lookupExpression(pce.getLeft(), ctx, deduceTypes2);
                            ...
                            if (best != null) {
                                ...
                                if (best instanceof FunctionDef) {
                                    ...
                                    if (parent instanceof NamespaceStatement) {
                                        invocation = deduceTypes2.phase.registerNamespaceInvocation(
                                                (NamespaceStatement) parent);
                                    ...
                                    deduceTypes2.forFunction(
                                            deduceTypes2.newFunctionInvocation(
                                                    (FunctionDef) best, pte, invocation, deduceTypes2.phase),
                                            new ForFunction() {
                                                @Override
                                                public void typeDecided(@NotNull final GenType aType) {
                                                    tte.setAttached(
                                                            deduceTypes2.gt(aType)); // TODO stop setting attached!
                ...
                case DOT_EXP:
                    {
                        ...
                        try {
                            final LookupResultList lrl =
                                    DeduceLookupUtils.lookupExpression(de.getLeft(), ctx, deduceTypes2);
                            @Nullable OS_Element best = lrl.chooseBest(null);
                            if (best != null) {
                                while (best instanceof AliasStatement) {
                                    best = DeduceLookupUtils._resolveAlias2((AliasStatement) best, deduceTypes2);
                        ...
                        if (el instanceof @NotNull final FunctionDef fd) {
                            ...
                            if (fd.getParent() == generatedFunction.getFD().getParent()) {
                                ...
                                if (fd.getParent() instanceof NamespaceStatement) {
                                    final NamespaceInvocation ni = deduceTypes2.phase.registerNamespaceInvocation(
                                            (NamespaceStatement) fd.getParent());
                                    invocation = ni;
                                        ...
                                        public void typeDecided(@NotNull final GenType aType) {
                                            if (!vte.typeDeferred_isPending()) {
                                                if (vte.resolvedType() == null) {
                                                    final @Nullable ClassInvocation ci =
                                                            deduceTypes2.genCI(aType, null);
