...
public class DeduceTypeResolve {
    ...
    private final DeferredObject<GenType, ResolveError, Void> typeResolution =
            new DeferredObject<GenType, ResolveError, Void>();
    ...
    public DeduceTypeResolve(final BaseTableEntry aBte) {
        ...
        if (bte instanceof IdentTableEntry) {
            ((IdentTableEntry) bte)
                    .backlinkSet()
                    .then((final InstructionArgument backlink0) -> __setBacklink_for_IdentTableEntry(backlink0));
            ...
            bte.addStatusListener(new BaseTableEntry.StatusListener() {
                ...
                public void onChange(final IElementHolder eh, final BaseTableEntry.Status newStatus) {
                    ...
                    if (!typeResolution.isPending()) {
                        ...
                        typeResolution.promise().then((xx) -> {
                            x[0] = xx;
                        });

                        SimplePrintLoggerToRemoveSoon.println_err(
                                "** typeResolution already resolved for " + eh.getElement() + " to " + x[0]);
                    } else {
                        if (!genType.isNull()) typeResolution.resolve(genType);
                    }
                }
                    ...
                    eh.getElement().visitGen(new AbstractCodeGen() {
                        ...
                        public void visitAliasStatement(final AliasStatement aAliasStatement) {
                            SimplePrintLoggerToRemoveSoon.println_err(String.format(
                                    "** AliasStatement %s points to %s%n",
                                    aAliasStatement.name(), aAliasStatement.getExpression()));
                        }
                        ...
                        public void visitFormalArgListItem(final @NotNull FormalArgListItem aFormalArgListItem) {
                            ...
                            } else throw new IllegalStateException("invalid entry (bte) " + bte);

                            final String s;
                            if (attached != null)
                                s = String.format(
                                        "** FormalArgListItem %s attached is not null. Type is %s. Points to %s%n",
                                        aFormalArgListItem.name(), aFormalArgListItem.typeName(), attached);
                            else
                                s = String.format(
                                        "** FormalArgListItem %s attached is null. Type is %s.%n",
                        ...
                        public void visitDefFunction(final DefFunctionDef aDefFunctionDef) {
                            SimplePrintLoggerToRemoveSoon.println_err(String.format(
                                    "** DefFunctionDef %s is %s%n",
                                    aDefFunctionDef.name(),
                                    ((StatementWrapper) aDefFunctionDef
                                                    .getItems()
                                                    .iterator()
                                                    .next())
                                            .getExpr()));
    ...
    private static void variableTableEntry_typeResolvePromise(
            final GenType result, final IElementHolder eh, final VariableTableEntry variableTableEntry) {
