...
class Assign_type_to_idte {
    ...
    private class ATTI_FoundElement extends FoundElement {
        ...
        public void foundElement(final OS_Element x) {
            if (ite.getResolvedElement() != x) ite.setStatus(BaseTableEntry.Status.KNOWN, new GenericElementHolder(x));
                    ...
                    cpte.typePromise().then(new DoneCallback<GenType>() {
                        ...
                        public void onDone(@NotNull final GenType result) {
                            SimplePrintLoggerToRemoveSoon.println2(
                                    "1483 " + result.getResolved() + " " + result.getNode());
                        }
                    });
                }
            }
