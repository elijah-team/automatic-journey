...
public class DeduceProcCall {
        ...
        if (t.getBacklink() == null) {
            ...
            if (bl_ instanceof IntegerIA) {
                ...
                if (resolved_element instanceof FormalArgListItem) {
                    target = new DeclTarget(
                            resolved_element, generatedFunction.getFD(), DeclAnchor.AnchorType.PARAMS, errSink);
                } else {
                    if (resolved_element instanceof VariableStatement) {
                        ...
                        if (resolved_element instanceof IdentExpression)
                            target = new DeclTarget(
                                    resolved_element, resolved_element, DeclAnchor.AnchorType.MEMBER, errSink);
                        else
                            target = new DeclTarget(
                                    resolved_element,
                                    resolved_element.getParent(),
                                    DeclAnchor.AnchorType.MEMBER,
                                    errSink);
                    }
                }
            }
    ...
    private class DeclTarget implements DeduceElement {
        ...
        public DeclTarget(
                final @NotNull OS_Element aBest,
            ...
            if (aAnchorType != DeclAnchor.AnchorType.VAR) {
                ...
                if (aAnchorType == DeclAnchor.AnchorType.INHERITED) {
                    ...
                    invocation = new DerivedClassInvocation(
                            (ClassStatement) aDeclAnchor, (ClassInvocation) declaredInvocation);
                } else {
                ...
                invocation =
                        ClassInvocationMake.withGenericPart(classStatement, null, normalTypeName, deduceTypes2.dcc());
            }
