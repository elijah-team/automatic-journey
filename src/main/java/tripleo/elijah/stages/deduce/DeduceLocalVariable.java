...
public class DeduceLocalVariable {
    ...
    static ClassStatement class_inherits(final ClassStatement aFirstClass, final OS_Element aInherited) {
        ...
        for (final Map.Entry<TypeName, ClassStatement> entry : inh1.entrySet()) {
            if (entry.getValue().equals(aInherited)) return (ClassStatement) aInherited;
        }
        return null;
    }

    public void setDeduceTypes2(
            final DeduceTypes2 aDeduceTypes2, final Context aContext, final BaseGeneratedFunction aGeneratedFunction) {
            ...
            }
            {
                ...
                if (genType.getTypeName() != null && genType.getTypeName() == genType.getResolved()) {
                    try {
                        genType.setResolved(deduceTypes2
                                .resolve_type(
                                        genType.getTypeName(), ctx /* genType.typeName.getTypeName().getContext() */)
                                .getResolved());
                        genType.genCIForGenType2(deduceTypes2);
                        vte.resolveType(genType);
                        vte.resolveTypeToClass(genType.getNode());
            ...
            vte.setStatus(BaseTableEntry.Status.KNOWN, new GenericElementHolder(vte.getResolvedElement()));
            {
                ...
                if (genType.getResolved() != null && genType.getNode() == null) {
                    if (genType.getResolved().getType() != OS_Type.Type.USER_CLASS
                        ...
                        try {
                            genType.setResolved(deduceTypes2
                                    .resolve_type(genType.getResolved(), ctx)
                                    .getResolved());
                        } catch (final ResolveError aResolveError) {
                            aResolveError.printStackTrace();
                            assert false;
                            ...
                            public void onDone(final @NotNull GeneratedClass result) {
                                ...
                                if (!vte.typePromise().isResolved()) { // HACK
                                    if (genType.getResolved() instanceof final OS_FuncType resolved) {
                                        result.functionMapDeferred(
                                                ((FunctionDef) resolved.getElement()), new FunctionMapDeferred() {
                                                    @Override
    ...
    public void resolve_var_table_entry_potential_types_1(
            final @NotNull VariableTableEntry vte, final BaseGeneratedFunction generatedFunction) {
        if (vte.potentialTypes().size() == 1) {
            ...
            if (tte1.tableEntry instanceof final ProcTableEntry procTableEntry) {
                ...
                if (dp.size() == 1) { // ia.getEntry().backlink == null
                    ...
                    switch (state) {
                        case 1:
                            ...
                            Self = new DeduceTypes2.OS_SpecialVariable(
                                    ((IntegerIA) self1).getEntry(), VariableTableType.SELF, generatedFunction);
                            break;
                        case 2:
                            Self = e_parent;
                            break;
                        case 3:
                            ...
                            Self = new DeduceTypes2.OS_SpecialVariable(
                                    ((IntegerIA) self2).getEntry(), VariableTableType.SELF, generatedFunction);
                            ((DeduceTypes2.OS_SpecialVariable) Self).memberInvocation =
                                    new MemberInvocation(b, MemberInvocation.Role.INHERITED);
                            break;
                        default:
                ...
                while (resolvedElement instanceof AliasStatement) {
                    try {
                        resolvedElement =
                                DeduceLookupUtils._resolveAlias2((AliasStatement) resolvedElement, deduceTypes2);
                ...
                if (resolvedElement != null) { // TODO feb 20
                    final @Nullable DeferredMemberFunction dm = deduceTypes2.deferred_member_function(
                            Self, null, (BaseFunctionDef) resolvedElement, procTableEntry.getFunctionInvocation());
