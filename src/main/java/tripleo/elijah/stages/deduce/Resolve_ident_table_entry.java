...
class Resolve_ident_table_entry {
    ...
    public void act(
            @NotNull final IdentTableEntry ite, final BaseGeneratedFunction generatedFunction, final Context ctx) {
        ...
        @Nullable InstructionArgument itex = new IdentIA(ite.getIndex(), generatedFunction);
        {
            while (itex != null && itex instanceof IdentIA) {
                ...
                if (x != null) {
                    ...
                    x.addStatusListener(new FoundParent(
                            deduceTypes2, x, itee, itee.getIdent().getContext(), generatedFunction)); // TODO
                    // context??
                }
            }
        }
