...
public class FoundParent implements BaseTableEntry.StatusListener {
	...
	@Contract(pure = true)
	public FoundParent(final DeduceTypes2 aDeduceTypes2, final BaseTableEntry aBte, final IdentTableEntry aIte,
			final Context aCtx, final BaseGeneratedFunction aGeneratedFunction) {
	...
	private void onChangeVTE(@NotNull final VariableTableEntry vte) {
		@NotNull
		final ArrayList<TypeTableEntry> pot = deduceTypes2.getPotentialTypesVte(vte);
		...
		final ErrSink errSink = deduceTypes2.errSink;
		@NotNull
		final DeducePhase phase = deduceTypes2.phase;

		if (vte.getStatus() == BaseTableEntry.Status.KNOWN && vte.type.getAttached() != null
...
//						ite.setStatus(BaseTableEntry.Status.KNOWN, best);
	...
	private void onChangeITE(final ITE_Zero zero, @NotNull final IdentTableEntry identTableEntry,
			final ErrSink errSink) {
		if (identTableEntry.type != null) {
			zero.fp_onChange__001(identTableEntry.type, this.ite, deduceTypes2, errSink);
		} else {
			if (!identTableEntry.fefi) {
				final Found_Element_For_ITE fefi = new Found_Element_For_ITE(generatedFunction, ctx, deduceTypes2.LOG,
						deduceTypes2.errSink, new DeduceTypes2.DeduceClient1(deduceTypes2));
				...
				identTableEntry.onFefiDone(new DoneCallback<GenType>() {
					...
					public void onDone(final GenType result) {
						...
						try {
							...
							final LookupResultList lrl = DeduceLookupUtils.lookupExpression(ite.getIdent(),
									resolvedClassOf.getContext(), deduceTypes2);
...
//			identTableEntry.setStatus(BaseTableEntry.Status.KNOWN, new GenericElementHolder(null));
	...
	private void postOnChange(@NotNull final IElementHolder eh) {
		if (ite.type == null && eh.getElement() instanceof VariableStatement) {
			@NotNull
			final TypeName typ = ((VariableStatement) eh.getElement()).typeName();
			@NotNull
			final OS_Type ty = new OS_UserType(typ);

			try {
				@Nullable
				final GenType ty2 = getTY2(typ, ty);
	...
	private void type_is_null_and_attached_is_null_vte() {
		...
		@NotNull
		final DeduceTypes2.PromiseExpectation<GenType> pe = deduceTypes2.promiseExpectation((VariableTableEntry) bte,
			...
			public void onDone(@NotNull final GenType result) {
				...
				if (attached1 != null) {
					...
					case USER:
						try {
							@NotNull
							final GenType ty3 = deduceTypes2.resolve_type(attached1,
