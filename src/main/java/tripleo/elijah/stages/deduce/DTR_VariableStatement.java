...
class DTR_VariableStatement {
    ...
    @Contract(pure = true)
    public DTR_VariableStatement(
            final DeduceTypeResolve aDeduceTypeResolve, final VariableStatement aVariableStatement) {
    ...
    private void normalTypeName_notGeneric(
            final IElementHolder eh, final GenType genType, final @NotNull NormalTypeName normalTypeName) {
        ...
        } else normalTypeName_notGeneric_typeNotProvided(genType, normalTypeName);
    }

    private void normalTypeName_generic_butNotNull(
            final IElementHolder eh, final @NotNull GenType genType, final NormalTypeName normalTypeName) {
        if (eh instanceof final GenericElementHolderWithType eh1) {
            final DeduceTypes2 dt2 = eh1.getDeduceTypes2();
            final OS_Type type = eh1.getType();
            ...
            try {
                final @NotNull GenType resolved =
                        dt2.resolve_type(genType.getTypeName(), variableStatement.getContext());
        ...
        } else genType.setTypeName(new OS_UserType(normalTypeName));
    }

    private void normalTypeName_notGeneric_typeProvided(
            final @NotNull GenType genType,
            final NormalTypeName normalTypeName,
    ...
    private /* static */ void normalTypeName_notGeneric_typeNotProvided(
            final @NotNull GenType genType, final NormalTypeName normalTypeName) {
        genType.setNonGenericTypeName(normalTypeName);
    }

    private /* static */ void normalTypeName_generic_butNotNull_resolveToGeneric(
            final GenType genType, final @NotNull GenType resolved, final @NotNull BaseTableEntry backlink) {
    ...
    private /* static */ void normalTypeName_generic_butNotNull_resolveToNonGeneric(
            final @NotNull GenType genType, final @NotNull GenType resolved) {
        genType.setResolved(resolved.getResolved());
    }
}
