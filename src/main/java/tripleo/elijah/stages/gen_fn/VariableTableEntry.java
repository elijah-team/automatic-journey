...
public class VariableTableEntry extends BaseTableEntry1
    ...
    public void addPotentialType(final int instructionIndex, final TypeTableEntry tte) {
        if (!typeDeferred.isPending()) {
            SimplePrintLoggerToRemoveSoon.println_err2(
                    "62 addPotentialType while typeDeferred is already resolved " + this); // throw new
            // AssertionError();
            return;
        }
        //
        if (!potentialTypes.containsKey(instructionIndex)) potentialTypes.put(instructionIndex, tte);
        else {
            ...
            if (v.getAttached() == null) {
                ...
                if ((tte.getAttached() != null && tte.getAttached().getType() != OS_Type.Type.USER)
                    ...
                    // other one
                    if (tte.getAttached() == null) tte.setAttached(v.getAttached());
                    else if (tte.getAttached().getType() == OS_Type.Type.USER_CLASS) v.setAttached(tte.getAttached());
                }
            }
        }
    }
    ...
    public void setConstructable(final ProcTableEntry aPte) {
        if (constructable_pte != aPte) {
            ...
            if (constructableDeferred.isPending()) constructableDeferred.resolve(constructable_pte);
        ...
        if (_resolveTypeCalled != null) { // TODO what a hack
            if (_resolveTypeCalled.getResolved() != null) {
                if (!aGenType.equals(_resolveTypeCalled)) {
                    System.err.printf(
                            "** 130 Attempting to replace %s with %s in %s%n",
    ...
    public PostBC_Processor getPostBC_Processor(
            final Context aFd_ctx, final DeduceTypes2.DeduceClient1 aDeduceClient1) {
