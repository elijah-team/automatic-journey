...
public class GenerateFunctions {
	...
	public void generateFromEntryPoints(final List<EntryPoint> entryPoints, final DeducePhase dp) {
		@NotNull
		final EntryPointList epl = new EntryPointList();
...
//		GeneratedFunction.printTables(gf);
	...
	private int add_i(@NotNull final BaseGeneratedFunction gf, final InstructionName x,
...
//	}
	...
	public GeneratedClass generateClass(final ClassStatement aClassStatement, final ClassInvocation aClassInvocation) {
		@NotNull
		final GeneratedClass Result = generateClass(aClassStatement);
		Result.ci = aClassInvocation;
		return Result;
	}

	@NotNull
	GeneratedFunction generateFunction(@NotNull final FunctionDef fd, final OS_Element parent,
			@NotNull final FunctionInvocation aFunctionInvocation) {
//		LOG.err("601.1 fn "+fd.name() + " " + parent);
		final GeneratedFunction gf = new GeneratedFunction(fd);
		if (parent instanceof ClassStatement)
	...
	public void generateAllTopLevelClasses(final List<GeneratedNode> lgc) {
		for (final ModuleItem item : module.getItems()) {
			if (item instanceof final NamespaceStatement namespaceStatement) {
				...
				@NotNull
				final GeneratedClass kl = generateClass(classStatement);
				lgc.add(kl);
			}
			// TODO enums, datatypes, (type)aliases
		}
...
//	}
	...
	private void generate_item_assignment(final StatementWrapper aStatementWrapper, @NotNull final IExpression x,
			@NotNull final BaseGeneratedFunction gf, final @NotNull Context cctx) {
//		LOG.err(String.format("801 %s %s", x.getLeft(), ((BasicBinaryExpression) x).getRight()));
		final BasicBinaryExpression bbe = (BasicBinaryExpression) x;
		final IExpression right1 = bbe.getRight();
	...
	InstructionArgument simplify_expression(@NotNull final IExpression expression,
			final @NotNull BaseGeneratedFunction gf, final Context cctx) {
		final ExpressionKind expressionKind = expression.getKind();
		switch (expressionKind) {
		case PROCEDURE_CALL:
			return simplify_expression_procedure_call(expression, gf, cctx);
		case CAST_TO: {
			...
			final InstructionArgument simp = simplify_expression(tce.getLeft(), gf, cctx);
			@NotNull
			final TypeTableEntry tte_index = gf.newTypeTableEntry(TypeTableEntry.Type.TRANSIENT,
...
//			break;
		case ASSIGNMENT: {
			final InstructionArgument s1 = simplify_expression(expression.getLeft(), gf, cctx);
			final InstructionArgument s2 = simplify_expression(((BasicBinaryExpression) expression).getRight(), gf,
					cctx);
			final int x = add_i(gf, InstructionName.AGN, List_of(s1, s2), cctx);
			return s1; // TODO is this right?
		}
...
//		return null;
	...
	@NotNull
	private InstructionArgument simplify_expression_procedure_call(@NotNull final IExpression expression,
...
//						throw new IllegalStateException("Cant be here");
	...
	private void generate_item_dot_expression(@org.jetbrains.annotations.Nullable final InstructionArgument backlink,
			final IExpression left, @NotNull final IExpression right, final @NotNull BaseGeneratedFunction gf,
			final Context cctx) {
		final int y = 2;
		final int x = gf.addIdentTableEntry((IdentExpression) left, cctx);
		if (backlink != null) {
...
//			gf.getIdentTableEntry(x).backlink = backlink;
	...
	private void assign_variable(final @NotNull BaseGeneratedFunction gf, final int vte,
			@NotNull final IExpression value, final Context cctx) {
	...
	private void simplify_procedure_call(@NotNull final ProcedureCallExpression pce,
			final @NotNull BaseGeneratedFunction gf, final Context cctx) {
		final IExpression left = pce.getLeft();
		final ExpressionList args = pce.getArgs();
		//
		...
		{
			@NotNull
			final ProcTableEntry pte = gf.getProcTableEntry(i);
			if (expression_num instanceof IdentIA) {
				@NotNull
				final IdentTableEntry idte = ((IdentIA) expression_num).getEntry();
				idte.setCallablePTE(pte);
				pte.typePromise().then(new DoneCallback<GenType>() { // TODO should this be done here?
					@Override
					public void onDone(final GenType result) {
						@NotNull
						final TypeTableEntry tte = gf.newTypeTableEntry(TypeTableEntry.Type.TRANSIENT, result.getResolved());
						tte.genType.copy(result);
						idte.addPotentialType(instructionIndex, tte);
					}
				});
	...
	private @NotNull List<InstructionArgument> simplify_args(
			@org.jetbrains.annotations.Nullable final ExpressionList args, final @NotNull BaseGeneratedFunction gf,
			final Context cctx) {
		final List<InstructionArgument> R = new ArrayList<InstructionArgument>();
		if (args == null)
			return R;
...
//				LOG.err("109 "+expression);
	...
	private @NotNull Collection<InstructionArgument> simplify_args2(
			@org.jetbrains.annotations.Nullable final ExpressionList args, @NotNull final GeneratedFunction gf,
			final Context cctx) {
		Collection<InstructionArgument> R = new ArrayList<InstructionArgument>();
		if (args == null)
			return R;
	...
	public int addProcTableEntry(final IExpression expression, final InstructionArgument expression_num,
			final List<TypeTableEntry> args, final BaseGeneratedFunction gf) {
		final ProcTableEntry pte = new ProcTableEntry(gf.prte_list.size(), expression, expression_num, args);
		gf.prte_list.add(pte);
	...
	@NotNull
	private Instruction expression_to_call_add_entry(final @NotNull BaseGeneratedFunction gf,
			@NotNull final ProcedureCallExpression pce, final @NotNull IExpression left, final Context cctx) {
		final Instruction i = new Instruction();
		i.setName(InstructionName.CALL); // TODO see line 686
		final List<InstructionArgument> li = new ArrayList<InstructionArgument>();
		final InstructionArgument assignment_path = gf.get_assignment_path(left, this, cctx);
	...
	List<TypeTableEntry> get_args_types(@org.jetbrains.annotations.Nullable final ExpressionList args,
			final BaseGeneratedFunction gf, @NotNull final Context aContext) {
		final List<TypeTableEntry> R = new ArrayList<TypeTableEntry>();
		if (args == null)
			return R;
		//
...
//			LOG.err(String.format("108 %s %s", arg, type));
	...
	private @NotNull Instruction expression_to_call(@NotNull final ProcedureCallExpression pce,
			@NotNull final BaseGeneratedFunction gf, @NotNull final Context cctx) {
		final IExpression left = pce.getLeft();
		switch (left.getKind()) {
		case IDENT: {
			return expression_to_call_add_entry(gf, pce, left, cctx);
...
//			simplify_qident((Qualident) pce.getLeft(), gf); // TODO ??
	...
	private int addTempTableEntry(final OS_Type type, @Nullable final IdentExpression name,
			@NotNull final BaseGeneratedFunction gf, @Nullable final OS_Element el) {
		final String theName;
		final int num;
		final TypeTableEntry tte;
	...
	private @NotNull InstructionArgument simplify_dot_expression(final DotExpression dotExpression,
			final @NotNull BaseGeneratedFunction gf, final Context cctx) {
		@NotNull
		final InstructionArgument x = gf.get_assignment_path(dotExpression, this, cctx);
		LOG.info("1117 " + x);
		return x;
	}

	private int addVariableTableEntry(final String name, final TypeTableEntry type,
			final @NotNull BaseGeneratedFunction gf, final OS_Element el) {
		return gf.addVariableTableEntry(name, VariableTableType.VAR, type, el);
	}
	...
	public static class S1toG_GC_Processor {
		...
		public int add_i(final GeneratedConstructor gf2, final InstructionName e, final List<InstructionArgument> lia,
				final Context cctx) {
			return gf.add_i(gf2, e, lia, cctx);
		}
...
//			throw new NotImplementedException();
		...
		private void generate_match_conditional(@NotNull final MatchConditional mc,
				final @NotNull BaseGeneratedFunction gf) {
			final int y = 2;
			final Context cctx = mc.getParent().getContext(); // TODO MatchConditional.getContext returns NULL!!!
			{
...
//			LOG.info("702 "+loop.getType());
		...
		private void generate_loop_FROM_TO_TYPE(@NotNull final Loop loop, @NotNull final BaseGeneratedFunction gf,
				final Context cctx) {
			final IdentExpression iterNameToken = loop.getIterNameToken();
			final String iterName = iterNameToken.getText();
			final int iter_temp = addTempTableEntry(null, iterNameToken, gf, iterNameToken); // TODO deduce later
...
//				LOG.info("705 "+statementItem);
		...
		private void generate_loop_EXPR_TYPE(@NotNull final Loop loop, @NotNull final BaseGeneratedFunction gf,
				final Context cctx) {
			final int loop_iterator = addTempTableEntry(null, gf); // TODO deduce later
			add_i(gf, InstructionName.DECL, List_of(new SymbolIA("tmp"), new IntegerIA(loop_iterator, gf)), cctx);
			final int i2 = addConstantTableEntry("", new NumericExpression(0),
...
//				add_i(gf, InstructionName.AGN, List_of(new IntegerIA(loop_iterator), ia1), cctx);
		...
		private void generate_variable_sequence(final VariableSequence item, @NotNull final BaseGeneratedFunction gf,
				final Context cctx) {
			for (final VariableStatement vs : item.items()) {
				int state = 0;
//				LOG.info("8004 " + vs);
...
//				LOG.info(String.format("8004-2 %s %s;", stype, vs.getName()));
		...
		private void generate_statement_wrapper(final StatementWrapper aStatementWrapper, final IExpression x,
				final ExpressionKind expressionKind, @NotNull final BaseGeneratedFunction gf, final Context cctx) {
//			LOG.err("106-1 "+x.getKind()+" "+x);
			if (x.is_simple()) {
//				int i = addTempTableEntry(x.getType(), gf);
				switch (expressionKind) {
...
//					break;
		...
		public void generate_construct_statement(final ConstructStatement aConstructStatement,
				@NotNull final BaseGeneratedFunction gf, final Context cctx) {
			final IExpression left = aConstructStatement.getExpr(); // TODO need type of this expr, not expr!!
			final ExpressionList args = aConstructStatement.getArgs();
			//
...

			{
				...
				if (expression_num instanceof IntegerIA || expression_num instanceof IdentIA
	...
	class Generate_item_assignment {
		...
		public void procedure_call(final StatementWrapper aStatementWrapper, @NotNull final BaseGeneratedFunction gf,
				final BasicBinaryExpression bbe, final ProcedureCallExpression pce, final Context cctx) {
			final IExpression left = bbe.getLeft();
			...
			if (lookup instanceof IntegerIA) {
				// TODO should be AGNC
				final int instruction_number = add_i(gf, InstructionName.AGN,
						List_of(lookup, new FnCallArgs(expression_to_call(pce, gf, cctx), gf)), cctx);
				final Instruction instruction = gf.getInstruction(instruction_number);
				final VariableTableEntry vte = gf.getVarTableEntry(((IntegerIA) lookup).getIndex());
				vte.addPotentialType(instruction.getIndex(), tte);
			} else {
				if (left instanceof IdentExpression) {
					...
					final int vte_num;
					if (aStatementWrapper instanceof WrappedStatementWrapper) {
						vte_num = addVariableTableEntry(text, tte, gf,
					...
					final Instruction instruction = gf.getInstruction(instruction_number);
					@NotNull
					final IdentTableEntry ite = ((IdentIA) lookup).getEntry();
					ite.addPotentialType(instruction.getIndex(), tte);
				}
			}
		}

		public void dot(@NotNull final BaseGeneratedFunction gf, final DotExpression left, final IExpression right,
				final Context cctx) {
		...
		public void ident(@NotNull final BaseGeneratedFunction gf, final @NotNull IdentExpression left,
				final IdentExpression right, final Context cctx) {
...
//					((IdentIA) some_left).getEntry().addPotentialType(inst, unknown_type);
		...
		public void mathematical(@NotNull final BaseGeneratedFunction gf, final @NotNull IExpression left,
				final ExpressionKind kind, final IExpression right1, final Context cctx) {
			// TODO doesn't use kind
			final TypeTableEntry tte = gf.newTypeTableEntry(TypeTableEntry.Type.SPECIFIED, right1.getType(), right1);
...
//				idte.type = tte;
		...
		public void string_literal(@NotNull final BaseGeneratedFunction gf, final IExpression left,
				final StringExpression right, final Context aContext) {
			@NotNull
		...
		public void neg(@NotNull final BaseGeneratedFunction gf, final IExpression left, final ExpressionKind aKind,
