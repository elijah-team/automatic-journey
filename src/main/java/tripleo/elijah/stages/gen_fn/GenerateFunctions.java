...
//		}
//
//		public int add_i(final GeneratedConstructor gf2, final InstructionName e, final List<InstructionArgument> lia, final
// Context cctx) {
...
//	                                                             final ClassStatement parent, // TODO Namespace
// constructors
//	                                                             final FunctionInvocation aFunctionInvocation) {
...
//			  gf.newTypeTableEntry(TypeTableEntry.Type.SPECIFIED, ((ClassStatement) parent).getOS_Type(),
// IdentExpression.forString("self")),
//			  null);
...
//							add_i(gf, InstructionName.IS_A, List_of(i, new IntegerIA(t.getIndex(), gf), /*TODO not*/new
// LabelIA(label_next)), cctx);
//							final Context context = mc1.getContext();
//
//							add_i(gf, InstructionName.DECL, List_of(new SymbolIA("tmp"), new IntegerIA(tmp, gf)), context);
//							final int cast_inst = add_i(gf, InstructionName.CAST_TO, List_of(new IntegerIA(tmp, gf), new
// IntegerIA(t.getIndex(), gf), (i)), context);
...
//		}
//
//		private void generate_loop_FROM_TO_TYPE(@NotNull final Loop loop, @NotNull final BaseGeneratedFunction gf, final
// Context cctx) {
...
//			add_i(gf, InstructionName.JE, List_of(new IntegerIA(iter_temp, gf), simplify_expression(loop.getToPart(), gf,
// cctx), label_bottom), cctx);
...
//		}
//
//		private void generate_loop_EXPR_TYPE(@NotNull final Loop loop, @NotNull final BaseGeneratedFunction gf, final
// Context cctx) {
...
//			gf.place(label_top);
//			final Label label_bottom = gf.addLabel("bottom"+label_top.getIndex(), false);
//			add_i(gf, InstructionName.JE, List_of(new IntegerIA(loop_iterator, gf), simplify_expression(loop.getToPart(), gf,
// cctx), label_bottom), cctx);
//			for (final StatementItem statementItem : loop.getItems()) {
...
//		}
//
//		private void generate_variable_sequence(final VariableSequence item, @NotNull final BaseGeneratedFunction gf, final
// Context cctx) {
...
//						final int vte_num = addVariableTableEntry(variable_name, gf.newTypeTableEntry(TypeTableEntry.Type.SPECIFIED,
// (initialValue.getType()), vs.getNameToken()), gf, vs.getNameToken());
...
//						final List<TypeTableEntry> argument_types = List_of(gf.getVarTableEntry(to_int(left)).type,
// gf.getVarTableEntry(to_int(right)).type);
...
//		}
//
//		public void generate_construct_statement(final ConstructStatement aConstructStatement, @NotNull final
// BaseGeneratedFunction gf, final Context cctx) {
...
//
//		public void procedure_call(final StatementWrapper aStatementWrapper, @NotNull final BaseGeneratedFunction gf, final
// BasicBinaryExpression bbe, final ProcedureCallExpression pce, final Context cctx) {
...
//						vte_num = addVariableTableEntry(text, tte, gf, ((WrappedStatementWrapper)
// aStatementWrapper).getVariableStatement());
//					} else {
...
//		}
//
//		public void ident(@NotNull final BaseGeneratedFunction gf, final IdentExpression left, final IdentExpression right,
// final Context cctx) {
...
//		}
//
//		public void numeric(@NotNull final BaseGeneratedFunction gf, final IExpression left, final NumericExpression ne,
// final Context cctx) {
...
//		}
//
//		public void mathematical(@NotNull final BaseGeneratedFunction gf, final IExpression left, final ExpressionKind kind,
// final IExpression right1, final Context cctx) {
...
//		}
//
//		public void string_literal(@NotNull final BaseGeneratedFunction gf, final IExpression left, final StringExpression
// right, final Context aContext) {
...
//		}
//
//		public void neg(@NotNull final BaseGeneratedFunction gf, final IExpression left, final ExpressionKind aKind, final
// IExpression right1, final Context cctx) {
//			// TODO doesn't use kind
//			final TypeTableEntry tte = gf.newTypeTableEntry(TypeTableEntry.Type.SPECIFIED, right1.getType(), right1);
//
//			final InstructionArgument left_ia = simplify_expression(left, gf, cctx);
...
// @NotNull final BaseGeneratedFunction gf, final Context cctx) {
...
//		else
//			generate_item_dot_expression(new IdentIA(x, gf), right.getLeft(), ((BasicBinaryExpression)right).getRight(), gf,
// cctx);
//	}
//
//	private void assign_variable(final @NotNull BaseGeneratedFunction gf, final int vte, @NotNull final IExpression
// value, final Context cctx) {
...
//				final int ii4 = add_i(gf, InstructionName.AGNF, List_of(new IntegerIA(vte, gf), new IntegerIA(pte_index, gf)),
// cctx);
//				final VariableTableEntry vte3_func = gf.getVarTableEntry(vte);
//				final TypeTableEntry tte_func = gf.newTypeTableEntry(TypeTableEntry.Type.TRANSIENT, new OS_FuncExprType(fe),
// value);
...
//	}
//
//	private void simplify_procedure_call(@NotNull final ProcedureCallExpression pce, final @NotNull BaseGeneratedFunction
// gf, final Context cctx) {
...
//	}
//
//	private @NotNull List<InstructionArgument> simplify_args(@org.jetbrains.annotations.Nullable final ExpressionList
// args, final @NotNull BaseGeneratedFunction gf, final Context cctx) {
...
//	}
//
//	private @NotNull Collection<InstructionArgument> simplify_args2(@org.jetbrains.annotations.Nullable final
// ExpressionList args, @NotNull final GeneratedFunction gf, final Context cctx) {
...
//	}
//
////	public int addProcTableEntry(final IExpression expression, final InstructionArgument expression_num, final
// List<TypeTableEntry> args, final BaseGeneratedFunction gf) {
////		final ProcTableEntry pte = new ProcTableEntry(gf.prte_list.size(), expression, expression_num, args);
////		gf.prte_list.add(pte);
////		return pte.index;
////	}
//
//	InstructionArgument simplify_expression(@NotNull final IExpression expression, final @NotNull BaseGeneratedFunction
// gf, final Context cctx) {
...
//				@NotNull final TypeTableEntry tte_index = gf.newTypeTableEntry(TypeTableEntry.Type.TRANSIENT, new
// OS_Type(tce.getTypeName()));
...
//
//	@NotNull
//	private InstructionArgument simplify_expression_procedure_call(@NotNull final IExpression expression, @NotNull final
// BaseGeneratedFunction gf, final Context cctx) {
...
//	}
//
//	private @NotNull InstructionArgument simplify_dot_expression(final DotExpression dotExpression, final @NotNull
// BaseGeneratedFunction gf, final Context cctx) {
...
//	//
//
//	private int addVariableTableEntry(final String name, final TypeTableEntry type, final @NotNull BaseGeneratedFunction
// gf, final OS_Element el) {
...
//	 */
//	private int addConstantTableEntry(final String name, final IExpression initialValue, final OS_Type type, final
...
//	 * @return the cte table index
//	 */
//	private int addConstantTableEntry2(final String name, final IExpression initialValue, final OS_Type type, final
// @NotNull BaseGeneratedFunction gf) {
...
//	// endregion
//
//	private int add_i(@NotNull final BaseGeneratedFunction gf, final InstructionName x, final List<InstructionArgument>
// list_of, final Context ctx) {
...
public class GenerateFunctions {
    ...
    public @NotNull GeneratedConstructor generateConstructor(
    ...
    public @NotNull GeneratedConstructor generateConstructor_000(
            final ConstructorDef aConstructorDef,
            final ClassStatement parent, // TODO Namespace constructors
            final FunctionInvocation aFunctionInvocation) {
        final GeneratedConstructor gf = new GeneratedConstructor(aConstructorDef);
...

        {
            ...
            for (int i = 0; i < fali_args.size(); i++) {
                ...
                final GenType genType = new GenType();
                final TypeName typeName = fali.typeName();
                if (typeName != null) genType.setTypeName(new OS_UserType(typeName));
                genType.setResolved(attached);

                final OS_Type attached1;
                if (attached == null && typeName != null) attached1 = genType.getTypeName();
    ...
    private int add_i(
            @NotNull final BaseGeneratedFunction gf,
            final InstructionName x,
            final List<InstructionArgument> list_of,
            final Context ctx) {
        final int i = gf.add(x, list_of, ctx);
        return i;
    }
    ...
    public @NotNull GeneratedClass generateClass(@NotNull final ClassStatement klass) {
        ...
        for (final ClassItem item : klass.getItems()) {
            if (item instanceof AliasStatement) {
                ...
                //				final ConstructorDef constructorDef = (ConstructorDef) item;
                //				@NotNull GeneratedConstructor f = generateConstructor(constructorDef, klass, null); // TODO remove
                // this null
                //				gc.addConstructor(constructorDef, f);
            } else if (item instanceof DestructorDef) {
                throw new NotImplementedException();
    ...
    GeneratedFunction generateFunction(
        ...
        final GeneratedFunction gf = new GeneratedFunction(fd);
        if (parent instanceof ClassStatement)
            gf.addVariableTableEntry(
                    "self",
                    VariableTableType.SELF,
                    gf.newTypeTableEntry(
                            TypeTableEntry.Type.SPECIFIED,
                            ((ClassStatement) parent).getOS_Type(),
                            IdentExpression.forString("self")),
                    null);
        final OS_Type returnType;
        final TypeName returnType1 = fd.returnType();
        if (returnType1 == null) returnType = new OS_UnitType();
        else returnType = new OS_UserType(returnType1);
        gf.addVariableTableEntry(
                "Result",
                VariableTableType.RESULT,
                gf.newTypeTableEntry(TypeTableEntry.Type.SPECIFIED, returnType, IdentExpression.forString("Result")),
    ...
    /*
     * public void generateFromEntryPoints(@NotNull List<EntryPoint> epl,
     * DeducePhase deducePhase) { new
     * EntryPointList(epl).generateFromEntryPoints(deducePhase, this, phase.wm); }
     */

    private void generate_item_assignment(
            final StatementWrapper aStatementWrapper,
            @NotNull final IExpression x,
            @NotNull final BaseGeneratedFunction gf,
    ...
    InstructionArgument simplify_expression(
            @NotNull final IExpression expression, final @NotNull BaseGeneratedFunction gf, final Context cctx) {
        final ExpressionKind expressionKind = expression.getKind();
        switch (expressionKind) {
            case PROCEDURE_CALL:
                return simplify_expression_procedure_call(expression, gf, cctx);
            case CAST_TO: {
                ...
                final InstructionArgument simp = simplify_expression(tce.getLeft(), gf, cctx);
                @NotNull
                final TypeTableEntry tte_index =
                        gf.newTypeTableEntry(TypeTableEntry.Type.TRANSIENT, new OS_UserType(tce.getTypeName()));
                final int x = add_i(
                        gf, InstructionName.CAST_TO, List_of(simp, new IntegerIA(tte_index.getIndex(), gf)), cctx);
            }
            case AS_CAST: {
                final TypeCastExpression tce = (TypeCastExpression) expression;
                final InstructionArgument simp = simplify_expression(tce.getLeft(), gf, cctx);
                ...
                final TypeTableEntry tte_index =
                        gf.newTypeTableEntry(TypeTableEntry.Type.TRANSIENT, new OS_UserType(tce.getTypeName()));
                final int x = add_i(
                        gf, InstructionName.AS_CAST, List_of(simp, new IntegerIA(tte_index.getIndex(), gf)), cctx);
            }
            case DOT_EXP: {
                final DotExpression de = (DotExpression) expression;
                return gf.get_assignment_path(de, this, cctx);
            ...
            case MODULO: {
                ...
                }
                {
                    ...
                    // TODO should be AGNC
                    final int x = add_i(gf, InstructionName.AGN, List_of(new IntegerIA(tmp, gf), fca), cctx);
                    return new IntegerIA(
                            tmp, gf); // TODO is this right?? we want to return the variable, not proc calls,
                    // right?
                }
                //				throw new NotImplementedException();
            }
            ...
            case ASSIGNMENT: {
                final InstructionArgument s1 = simplify_expression(expression.getLeft(), gf, cctx);
                final InstructionArgument s2 =
                        simplify_expression(((BasicBinaryExpression) expression).getRight(), gf, cctx);
                final int x = add_i(gf, InstructionName.AGN, List_of(s1, s2), cctx);
                return s1; // TODO is this right?
            }
            default:
    ...
    @NotNull
    private InstructionArgument simplify_expression_procedure_call(
            @NotNull final IExpression expression, @NotNull final BaseGeneratedFunction gf, final Context cctx) {
        final @NotNull ProcedureCallExpression pce = (ProcedureCallExpression) expression;
        final IExpression left = pce.getLeft();
        final ExpressionList args = pce.getArgs();
        final InstructionArgument left_ia;
        ...
        if (args != null) {
            for (final IExpression arg : args) {
                ...
                if (arg.is_simple()) {
                    ...
                    ia = simplify_expression(arg, gf, cctx);
                    iat = gf.newTypeTableEntry(
                            TypeTableEntry.Type.TRANSIENT, null /* README wait to be deduced */, arg);
                }
                right_ia.add(ia);
                args1.add(iat);
            }
        ...
        right_ia.add(0, new ProcIA(pte, gf));
        {
            ...
            // TODO should be AGNC
            final int x =
                    add_i(gf, InstructionName.AGN, List_of(new IntegerIA(tmp_var, gf), new FnCallArgs(i, gf)), cctx);
            return new IntegerIA(tmp_var, gf); // return tmp_var instead of expression assigning it
        }
    }

    private void generate_item_dot_expression(
            @org.jetbrains.annotations.Nullable final InstructionArgument backlink,
            final IExpression left,
            @NotNull final IExpression right,
            final @NotNull BaseGeneratedFunction gf,
            final Context cctx) {
        ...
        if (right.getLeft() == right) return;
        //
        if (right instanceof IdentExpression)
            generate_item_dot_expression(new IdentIA(x, gf), right.getLeft(), right, gf, cctx);
        else
            generate_item_dot_expression(
                    new IdentIA(x, gf), right.getLeft(), ((BasicBinaryExpression) right).getRight(), gf, cctx);
    }

    private void assign_variable(
            final @NotNull BaseGeneratedFunction gf,
            final int vte,
            @NotNull final IExpression value,
            final Context cctx) {
        if (value == IExpression.UNASSIGNED) return; // default_expression
        switch (value.getKind()) {
            case PROCEDURE_CALL:
            ...
            case NUMERIC:
                final int ci = addConstantTableEntry(null, value, value.getType(), gf);
                final int ii = add_i(
                        gf, InstructionName.AGNK, List_of(new IntegerIA(vte, gf), new ConstTableIA(ci, gf)), cctx);
                final VariableTableEntry vte_numeric = gf.getVarTableEntry(vte);
                vte_numeric.addPotentialType(ii, gf.getConstTableEntry(ci).type);
                break;
            case IDENT:
            ...
            case FUNC_EXPR:
                ...
                final int pte_index = addProcTableEntry(fe, null, get_args_types(fe.getArgs(), gf), gf);
                final int ii4 = add_i(
                        gf, InstructionName.AGNF, List_of(new IntegerIA(vte, gf), new IntegerIA(pte_index, gf)), cctx);
                final VariableTableEntry vte3_func = gf.getVarTableEntry(vte);
                final TypeTableEntry tte_func =
                        gf.newTypeTableEntry(TypeTableEntry.Type.TRANSIENT, new OS_FuncExprType(fe), value);
                vte3_func.addPotentialType(ii4, tte_func);
                break;
            default:
                throw new IllegalStateException("Unexpected value: " + value.getKind());
    ...
    private void simplify_procedure_call(
            @NotNull final ProcedureCallExpression pce, final @NotNull BaseGeneratedFunction gf, final Context cctx) {
        final IExpression left = pce.getLeft();
        final ExpressionList args = pce.getArgs();
        //
        InstructionArgument expression_num = simplify_expression(left, gf, cctx);
        ...
        if (left instanceof ProcedureCallExpression)
            i = addProcTableEntry(left, expression_num, get_args_types(args, gf, cctx), gf);
        else i = addProcTableEntry(pce, expression_num, get_args_types(args, gf, cctx), gf);
        ...
        final int instructionIndex = add_i(gf, InstructionName.CALL, l, cctx);
        {
            ...
            if (expression_num instanceof IdentIA) {
                ...
                pte.typePromise()
                        .then(
                                new DoneCallback<GenType>() { // TODO should this be done here?
                                    @Override
                                    public void onDone(final GenType result) {
                                        @NotNull
                                        final TypeTableEntry tte = gf.newTypeTableEntry(
                                                TypeTableEntry.Type.TRANSIENT, result.getResolved());
                                        tte.genType.copy(result);
                                        idte.addPotentialType(instructionIndex, tte);
                                    }
                                });
    ...
    private @NotNull List<InstructionArgument> simplify_args(
            @org.jetbrains.annotations.Nullable final ExpressionList args,
            final @NotNull BaseGeneratedFunction gf,
            final Context cctx) {
        final List<InstructionArgument> R = new ArrayList<InstructionArgument>();
        if (args == null) return R;
    ...
    public int addProcTableEntry(
            final IExpression expression,
            final InstructionArgument expression_num,
            final List<TypeTableEntry> args,
            final BaseGeneratedFunction gf) {
        final ProcTableEntry pte = new ProcTableEntry(gf.prte_list.size(), expression, expression_num, args);
        gf.prte_list.add(pte);
        if (expression_num instanceof final IdentIA identIA) {
            if (identIA.getEntry().getCallablePTE() == null) identIA.getEntry().setCallablePTE(pte);
        }
        return pte.index;
    }

    @NotNull
    private Instruction expression_to_call_add_entry(
            final @NotNull BaseGeneratedFunction gf,
            @NotNull final ProcedureCallExpression pce,
            final @NotNull IExpression left,
            final Context cctx) {
    ...
    @NotNull
    List<TypeTableEntry> get_args_types(
            @org.jetbrains.annotations.Nullable final ExpressionList args,
            final BaseGeneratedFunction gf,
            @NotNull final Context aContext) {
        final List<TypeTableEntry> R = new ArrayList<TypeTableEntry>();
        ...
        for (final IExpression arg : args) {
            ...
            if (arg instanceof IdentExpression) {
                ...
                R.add(tte);
            } else R.add(getType(arg, gf));
        }
        assert R.size() == args.size();
        return R;
    }

    private @NotNull List<TypeTableEntry> get_args_types(
            @NotNull final List<FormalArgListItem> args, final @NotNull BaseGeneratedFunction gf) {
        final List<TypeTableEntry> R = new ArrayList<TypeTableEntry>();
        //
        for (final FormalArgListItem arg : args) {
            final TypeTableEntry tte;
            final OS_UserType ty;
            if (arg.typeName() == null || arg.typeName().isNull()) ty = null;
            else ty = new OS_UserType(arg.typeName());
    ...
    private @NotNull Instruction expression_to_call(
            @NotNull final ProcedureCallExpression pce,
            @NotNull final BaseGeneratedFunction gf,
            @NotNull final Context cctx) {
        final IExpression left = pce.getLeft();
        switch (left.getKind()) {
            ...
            case DOT_EXP: {
                @NotNull
                final InstructionArgument x = simplify_dot_expression((DotExpression) left, gf, cctx); // TODO ??
                return expression_to_call_add_entry(gf, pce, left, x, cctx);
            }
            case PROCEDURE_CALL: {
                final int y = 2;
     ...
     * @return the cte table index
     */
    private int addConstantTableEntry(
            final String name,
     ...
     * @return the cte table index
     */
    private int addConstantTableEntry2(
            final String name,
            final IExpression initialValue,
            final OS_Type type,
            final @NotNull BaseGeneratedFunction gf) {
    ...
    private @NotNull InstructionArgument simplify_dot_expression(
            final DotExpression dotExpression, final @NotNull BaseGeneratedFunction gf, final Context cctx) {
        @NotNull final InstructionArgument x = gf.get_assignment_path(dotExpression, this, cctx);
        LOG.info("1117 " + x);
        return x;
    }

    private int addVariableTableEntry(
            final String name,
    ...
    public static class S1toG_GC_Processor {
        ...
        public int add_i(
                final GeneratedConstructor gf2,
                final InstructionName e,
                final List<InstructionArgument> lia,
                final Context cctx) {
            return gf.add_i(gf2, e, lia, cctx);
        }
    ...
    class Generate_Item {
        ...
        private void generate_match_conditional(
                @NotNull final MatchConditional mc, final @NotNull BaseGeneratedFunction gf) {
            final int y = 2;
            final Context cctx = mc.getParent().getContext(); // TODO MatchConditional.getContext returns NULL!!!
            {
                final IExpression expr = mc.getExpr();
...

                {
                    for (final MatchConditional.MC1 part : mc.getParts()) {
                        if (part instanceof final MatchConditional.MatchArm_TypeMatch mc1) {
                            ...
                            add_i(
                                    gf,
                                    InstructionName.DECL,
                                    List_of(new SymbolIA("tmp"), new IntegerIA(tmp, gf)),
                                    context);
                            final int cast_inst = add_i(
                                    gf,
                                    InstructionName.CAST_TO,
                                    List_of(new IntegerIA(tmp, gf), new IntegerIA(t.getIndex(), gf), (i)),
                                    context);
        ...
        private void generate_if(@NotNull final IfConditional ifc, final @NotNull BaseGeneratedFunction gf) {
            ...
            final Label label_end = gf.addLabel();
            {
                ...
                final InstructionArgument i = simplify_expression(expr, gf, cctx);
                //				LOG.info("711 " + i);
                final int const_true =
                        addConstantTableEntry("true", Boolean_true, new OS_BuiltinType(BuiltInTypes.Boolean), gf);
                add_i(gf, InstructionName.JNE, List_of(i, new ConstTableIA(const_true, gf), label_next), cctx);
                final int begin_1st = add_i(gf, InstructionName.ES, null, cctx);
                final int begin_2nd = add_i(gf, InstructionName.ES, null, cctx);
                for (final OS_Element item : ifc.getItems()) {
                    ...
                    for (final IfConditional part : parts) {
                        ...
                        if (part.getExpr() != null) {
                            ...
                            add_i(
                                    gf,
                                    InstructionName.JNE,
                                    List_of(ii, new ConstTableIA(const_true, gf), label_next),
                                    cctx);
                        }
                        final int begin_next = add_i(gf, InstructionName.ES, null, cctx);
                        for (final OS_Element partItem : part.getItems()) {
        ...
        private void generate_loop_FROM_TO_TYPE(
                @NotNull final Loop loop, @NotNull final BaseGeneratedFunction gf, final Context cctx) {
            final IdentExpression iterNameToken = loop.getIterNameToken();
            final String iterName = iterNameToken.getText();
            final int iter_temp = addTempTableEntry(null, iterNameToken, gf, iterNameToken); // TODO deduce later
            add_i(gf, InstructionName.DECL, List_of(new SymbolIA("tmp"), new IntegerIA(iter_temp, gf)), cctx);
            ...
            if (ia1 instanceof ConstTableIA)
                add_i(gf, InstructionName.AGNK, List_of(new IntegerIA(iter_temp, gf), ia1), cctx);
            else add_i(gf, InstructionName.AGN, List_of(new IntegerIA(iter_temp, gf), ia1), cctx);
            final Label label_top = gf.addLabel("top", true);
            gf.place(label_top);
            final Label label_bottom = gf.addLabel("bottom" + label_top.getIndex(), false);
            add_i(
                    ...
                    InstructionName.JE,
                    List_of(
                            new IntegerIA(iter_temp, gf),
                            simplify_expression(loop.getToPart(), gf, cctx),
                            label_bottom),
                    cctx);
            for (final StatementItem statementItem : loop.getItems()) {
                //				LOG.info("705 "+statementItem);
            ...
            final TypeTableEntry tte = gf.newTypeTableEntry(TypeTableEntry.Type.TRANSIENT, null, pre_inc_name);
            final int pre_inc =
                    addProcTableEntry(pre_inc_name, null, List_of(tte /* getType(left), getType(right) */), gf);
            add_i(gf, InstructionName.CALLS, List_of(new ProcIA(pre_inc, gf), new IntegerIA(iter_temp, gf)), cctx);
            add_i(gf, InstructionName.JMP, List_of(label_top), cctx);
            gf.place(label_bottom);
        }

        private void generate_loop_EXPR_TYPE(
                @NotNull final Loop loop, @NotNull final BaseGeneratedFunction gf, final Context cctx) {
            final int loop_iterator = addTempTableEntry(null, gf); // TODO deduce later
            add_i(gf, InstructionName.DECL, List_of(new SymbolIA("tmp"), new IntegerIA(loop_iterator, gf)), cctx);
            final int i2 = addConstantTableEntry(
                    "", new NumericExpression(0), new OS_BuiltinType(BuiltInTypes.SystemInteger), gf);
            final InstructionArgument ia1 = new ConstTableIA(i2, gf);
            //			if (ia1 instanceof ConstTableIA)
            add_i(gf, InstructionName.AGNK, List_of(new IntegerIA(loop_iterator, gf), ia1), cctx);
            //			else
            ...
            add_i(
                    ...
                    InstructionName.JE,
                    List_of(
                            new IntegerIA(loop_iterator, gf),
                            simplify_expression(loop.getToPart(), gf, cctx),
                            label_bottom),
                    cctx);
            for (final StatementItem statementItem : loop.getItems()) {
        ...
        private void generate_variable_sequence(
                final VariableSequence item, @NotNull final BaseGeneratedFunction gf, final Context cctx) {
            for (final VariableStatement vs : item.items()) {
                int state = 0;
                //				LOG.info("8004 " + vs);
                final String variable_name = vs.getName();
                ...
                switch (state) {
                    case 1:
                        {
                            final int ci =
                                    addConstantTableEntry(variable_name, initialValue, initialValue.getType(), gf);
                            final int vte_num = addVariableTableEntry(
                                    variable_name,
                                    gf.newTypeTableEntry(
                                            TypeTableEntry.Type.SPECIFIED, (initialValue.getType()), vs.getNameToken()),
                                    gf,
                                    vs.getNameToken());
                            final IExpression iv = initialValue;
                            add_i(
                                    gf,
                                    InstructionName.DECL,
                                    List_of(new SymbolIA("const"), new IntegerIA(vte_num, gf)),
                                    cctx);
                            add_i(
                                    gf,
                                    InstructionName.AGNK,
                                    List_of(new IntegerIA(vte_num, gf), new ConstTableIA(ci, gf)),
                                    cctx);
                        }
                        break;
                    case 2:
                        {
                            ...
                            add_i(
                                    gf,
                                    InstructionName.DECL,
                                    List_of(new SymbolIA("val"), new IntegerIA(vte_num, gf)),
                                    cctx);
                            final IExpression iv = initialValue;
                            assign_variable(gf, vte_num, iv, cctx);
                        }
                    ...
                    case 3:
                        {
                            final TypeTableEntry tte;
                            if (initialValue == IExpression.UNASSIGNED && vs.typeName() != null) {
                                tte = gf.newTypeTableEntry(
                                        TypeTableEntry.Type.SPECIFIED,
                                        new OS_UserType(vs.typeName()),
                                        vs.getNameToken());
                            } else {
                                tte = gf.newTypeTableEntry(
                                        TypeTableEntry.Type.SPECIFIED, initialValue.getType(), vs.getNameToken());
                            ...
                            add_i(
                                    gf,
                                    InstructionName.DECL,
                                    List_of(new SymbolIA("var"), new IntegerIA(vte_num, gf)),
                                    cctx);
                            final IExpression iv = initialValue;
                            assign_variable(gf, vte_num, iv, cctx);
                        }
        ...
        private void generate_statement_wrapper(
                final StatementWrapper aStatementWrapper,
                final IExpression x,
                final ExpressionKind expressionKind,
                @NotNull final BaseGeneratedFunction gf,
            ...
            if (x.is_simple()) {
                ...
                switch (expressionKind) {
                    case ASSIGNMENT:
                        //					LOG.err(String.format("703.2 %s %s", x.getLeft(),
                    ...
                    case AUG_MULT:
                        {
                            LOG.info(String.format(
                                    "801.1 %s %s %s",
                                    expressionKind, x.getLeft(), ((BasicBinaryExpression) x).getRight()));
                            //						BasicBinaryExpression bbe = (BasicBinaryExpression) x;
                            //						final IExpression right1 = bbe.getRight();
                            final InstructionArgument left = simplify_expression(x.getLeft(), gf, cctx);
                            final InstructionArgument right =
                                    simplify_expression(((BasicBinaryExpression) x).getRight(), gf, cctx);
                            final IdentExpression fn_aug_name =
                                    Helpers.string_to_ident(SpecialFunctions.of(expressionKind));
                            final List<TypeTableEntry> argument_types = List_of(
                                    gf.getVarTableEntry(to_int(left)).type, gf.getVarTableEntry(to_int(right)).type);
                            //						LOG.info("801.2 "+argument_types);
                            final int fn_aug = addProcTableEntry(fn_aug_name, null, argument_types, gf);
                            final int i = add_i(
                                    gf, InstructionName.CALLS, List_of(new ProcIA(fn_aug, gf), left, right), cctx);
                            //
                            // SEE IF CALL SHOULD BE DEFERRED
                            //
                ...
                switch (expressionKind) {
                    case ASSIGNMENT:
                        //					LOG.err(String.format("803.2 %s %s", x.getLeft(),
                        // ((BasicBinaryExpression)x).getRight()));
                        generate_item_assignment(aStatementWrapper, x, gf, cctx);
                        break;
                        //				case IS_A:
        ...
        public void generate_construct_statement(
                final ConstructStatement aConstructStatement,
                @NotNull final BaseGeneratedFunction gf,
                final Context cctx) {
            final IExpression left = aConstructStatement.getExpr(); // TODO need type of this expr, not expr!!
            final ExpressionList args = aConstructStatement.getArgs();
...

            {
                ...
                if (expression_num instanceof IntegerIA
                        || expression_num instanceof IdentIA
                                && ((IdentIA) expression_num).getEntry().backlink == null) {
                } else {
    ...
    class Generate_item_assignment {
        ...
        public void procedure_call(
                final StatementWrapper aStatementWrapper,
                @NotNull final BaseGeneratedFunction gf,
                final BasicBinaryExpression bbe,
            ...
            if (lookup instanceof IntegerIA) {
                // TODO should be AGNC
                final int instruction_number = add_i(
                        gf,
                        InstructionName.AGN,
                        List_of(lookup, new FnCallArgs(expression_to_call(pce, gf, cctx), gf)),
                        cctx);
                final Instruction instruction = gf.getInstruction(instruction_number);
                final VariableTableEntry vte = gf.getVarTableEntry(((IntegerIA) lookup).getIndex());
                vte.addPotentialType(instruction.getIndex(), tte);
                ...
                if (left instanceof IdentExpression) {
                    ...
                    final int vte_num;
                    if (aStatementWrapper instanceof WrappedStatementWrapper) {
                        vte_num = addVariableTableEntry(
                                text, tte, gf, ((WrappedStatementWrapper) aStatementWrapper).getVariableStatement());
                    } else {
                        vte_num = addVariableTableEntry(text, tte, gf, (IdentExpression) left);
                    }
                    add_i(gf, InstructionName.DECL, List_of(new SymbolIA("tmp"), new IntegerIA(vte_num, gf)), cctx);
                    ...
                    final int instruction_number = add_i(
                            gf,
                            InstructionName.AGN,
                            List_of(new IntegerIA(vte_num, gf), new FnCallArgs(expression_to_call(pce, gf, cctx), gf)),
                            cctx);
                    final Instruction instruction = gf.getInstruction(instruction_number);
                    final VariableTableEntry vte = gf.getVarTableEntry(vte_num);
                    vte.addPotentialType(instruction.getIndex(), tte);
        ...
        public void ident(
                @NotNull final BaseGeneratedFunction gf,
                final @NotNull IdentExpression left,
                final IdentExpression right,
                final Context cctx) {
        ...
        public void string_literal(
                @NotNull final BaseGeneratedFunction gf,
                final IExpression left,
                final StringExpression right,
                final Context aContext) {
            @NotNull
            final InstructionArgument agn_path = gf.get_assignment_path(left, GenerateFunctions.this, aContext);
            final int cte = addConstantTableEntry(
                    "", right, new OS_BuiltinType(BuiltInTypes.String_) /* right.getType() */, gf);
