...
public class OutputStrategyC {
    ...
    public String nameForFunction(final @NotNull GeneratedFunction generatedFunction, final GenerateResult.TY aTy) {
        ...
        if (c instanceof GeneratedClass) return nameForClass((GeneratedClass) c, aTy);
        else if (c instanceof GeneratedNamespace) return nameForNamespace((GeneratedNamespace) c, aTy);
        return null;
    }
    ...
    public @NotNull String nameForClass(final GeneratedClass generatedClass, final GenerateResult.TY aTy) {
        ...
        if (lsp == null) sb.append("______________");
        else
        ...
        if (pkg != OS_Package.default_package) {
            if (pkg == null) pkg = findPackage(generatedClass.getKlass());
        ...
        switch (outputStrategy.per()) {
            case PER_CLASS:
                {
                    if (generatedClass.isGeneric()) sb.append(generatedClass.getNumberedName());
                    else sb.append(generatedClass.getName());
                }
                break;
            case PER_MODULE:
                {
    ...
    public String nameForNamespace(final GeneratedNamespace generatedNamespace, final GenerateResult.TY aTy) {
        ...
        } else filename = generatedNamespace.getName();
        final StringBuilder sb = new StringBuilder();
        sb.append("/");
        final LibraryStatementPart lsp = generatedNamespace.module().getLsp();
        if (lsp == null) sb.append("___________________");
        else sb.append(lsp.getInstructions().getName());
        sb.append("/");
        OS_Package pkg = generatedNamespace.getNamespaceStatement().getPackageName();
        if (pkg != OS_Package.default_package) {
            if (pkg == null) pkg = findPackage(generatedNamespace.getNamespaceStatement());
            sb.append(pkg.getName());
            sb.append("/");
        }
        sb.append(filename);
    ...
    private OS_Package findPackage(OS_Element e) {
        while (e != null) {
            e = e.getParent();
            if (e.getContext().getParent() == e.getContext()) e = null;
            else {
                ...
                switch (t) {
                    case NAMESPACE:
                        ...
                        break;
                    case CLASS:
                        if (((ClassStatement) e).getPackageName() != null) return ((ClassStatement) e).getPackageName();
                        break;
                    case FUNCTION:
                        continue;
                    default:
    ...
    public String nameForConstructor(
            final GeneratedConstructor generatedConstructor, final GenerateResult.@NotNull TY aTy) {
