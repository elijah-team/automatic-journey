...
public class MainClassEntryPoint implements EntryPoint {
    ...
    public MainClassEntryPoint(final ClassStatement aKlass) {
        ...
        for (final ClassItem classItem : main) {
            ...
            if (typeName == null) return_type_is_null = true;
            else return_type_is_null = typeName.isNull();
            if (fd.getArgs().size() == 0 && return_type_is_null) {
                main_function = fd;
            }
        }
        if (main_function == null) throw new IllegalArgumentException("Class does not define main");
        klass = aKlass;
    }
    ...
    public static boolean isMainClass(@NotNull final ClassStatement classStatement) {
        ...
        return classStatement.getPackageName() == OS_Package.default_package
                && classStatement.name().equals("Main");
    }
