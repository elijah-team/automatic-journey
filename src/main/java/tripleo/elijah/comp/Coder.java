...
public class Coder {
    ...
    private static void extractNodes_toResolvedNodes(
            @NotNull final Map<FunctionDef, GeneratedFunction> aFunctionMap,
            @NotNull final List<GeneratedNode> resolved_nodes) {
        aFunctionMap.values().stream()
                .map(generatedFunction -> (generatedFunction.idte_list)
                        .stream()
    ...
    public void codeNodes(
            final OS_Module mod, final List<GeneratedNode> resolved_nodes, final GeneratedNode generatedNode) {
        if (generatedNode instanceof final GeneratedFunction generatedFunction) {
            ...
            if (generatedClass.getCode() == 0) codeNodeClass(generatedClass, mod);
            ...
            if (generatedNamespace.getCode() != 0) codeNodeNamespace(generatedNamespace, mod);
    ...
    private void setClassmapNodeCodes(
            @NotNull final Map<ClassStatement, GeneratedClass> aClassMap, final OS_Module mod) {
