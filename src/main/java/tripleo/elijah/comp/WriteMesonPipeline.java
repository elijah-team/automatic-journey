...
public class WriteMesonPipeline implements PipelineMember, @NotNull Consumer<Supplier<GenerateResult>> {
    ...
    DoubleLatch<Multimap<CompilerInstructions, String>> write_makefiles_latch =
            new DoubleLatch<>(this::write_makefiles_action);
    ...
    private void write_root(
            @NotNull final Multimap<CompilerInstructions, String> lsp_outputs, final List<String> aDep_dirs)
            throws IOException {
        final CharSink root_file = c.getIO().openWrite(getPath("meson.build"));
        try {
            final String project_name = c.getProjectName();
            final String project_string =
                    String.format("project('%s', 'c', version: '1.0.0', meson_version: '>= 0.48.0',)", project_name);
            ...
            final String deps_names = String_join(
                    ", ",
                    aDep_dirs.stream()
                            .map(x -> String.format("%s", x)) // TODO _lib
                            // ??
                            .collect(Collectors.toList()));
            root_file.accept(String.format(
                    "%s_bin = executable('%s', link_with: [ %s ], install: true)",
    ...
    private Path getPath(final String aName) {
        return FileSystems.getDefault().getPath("COMP", c.getCompilationNumberString(), aName);
    }

    private void write_lsp(
            @NotNull final Multimap<CompilerInstructions, String> lsp_outputs,
            final CompilerInstructions compilerInstructions,
            final String aSub_dir)
            throws IOException {
        final Path path =
                FileSystems.getDefault().getPath("COMP", c.getCompilationNumberString(), aSub_dir, "meson.build");
        ...
        try {
            ...
            sub_file.accept("\n");
            sub_file.accept(String.format(
                    "%s = static_library('%s', %s_sources, install: false,)",
                    aSub_dir, aSub_dir, aSub_dir)); // include_directories, dependencies: [],
            sub_file.accept("\n");
            sub_file.accept("\n");
            sub_file.accept(String.format(
                    "%s_dep = declare_dependency( link_with: %s )", aSub_dir, aSub_dir)); // include_directories
            sub_file.accept("\n");
        } finally {
    ...
    private void write_prelude() throws IOException {
        ...
        try {
            ...
            final List<String> files = List_of("'Prelude.c'") /*
									 * files_.stream() .filter(x -> x.endsWith(".c")) .map(x ->
									 * String.format("\t'%s',", x)) .collect(Collectors.toList())
									 */;
            ...
            prel_file.accept("\n");
            prel_file.accept("\n");
            prel_file.accept(String.format(
                    "%s_dep = declare_dependency( link_with: %s )", "Prelude", "Prelude")); // include_directories
            prel_file.accept("\n");
        } finally {
    ...
    private void write_makefiles() {
        // Multimap<CompilerInstructions, String> lsp_outputs =
        // writePipeline.getLspOutputs(); // TODO move this
            ...
            public void accept(final Supplier<GenerateResult> aGenerateResultSupplier) {
                if (grs != null) {
                    SimplePrintLoggerToRemoveSoon.println_err2(
                            "234 grs not null " + grs.getClass().getName());
                    return;
                }
