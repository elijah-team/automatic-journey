...
public class DeduceLookupUtils {
	...
	@Nullable
	public static OS_Element _resolveAlias2(final @NotNull AliasStatement aliasStatement,
			@NotNull final DeduceTypes2 deduceTypes2) throws ResolveError {
...
//			s.pop();
	...
	public static @Nullable GenType deduceExpression(@NotNull final DeduceTypes2 aDeduceTypes2,
			@NotNull final IExpression n, final @NotNull Context context) throws ResolveError {
		switch (n.getKind()) {
		case IDENT:
			final Promise<GenType, ResolveError, Void> x = deduceIdentExpression_p(aDeduceTypes2, (IdentExpression) n,
					context);
		...
		case PROCEDURE_CALL:
			@Nullable
			final GenType result = new GenType();
	...
	public static Promise<GenType, ResolveError, Void> deduceExpression_p(@NotNull final DeduceTypes2 aDeduceTypes2,
			@NotNull final IExpression n, final @NotNull Context context) {
	...
	private static Promise<@Nullable GenType, ResolveError, Void> deduceProcedureCall_p(
			final @NotNull ProcedureCallExpression pce, final @NotNull Context ctx,
	...
	private static Promise<@Nullable GenType, ResolveError, Void> deduceIdentExpression_p(
			@NotNull final DeduceTypes2 aDeduceTypes2, final @NotNull IdentExpression ident,
	...
	static @Nullable OS_Element lookup(@NotNull final IExpression expression, @NotNull final Context ctx,
			@NotNull final DeduceTypes2 deduceTypes2) throws ResolveError {
		...
		case DOT_EXP:
			...
			@Nullable
			final OS_Element best3 = lrl3.chooseBest(null);
			return best3;
//		default:
	...
	static class __deduceExpression_p {
		...
		public void do_deduceExpression(final DeduceTypes2 aDeduceTypes2, final IExpression n, final Context context) {
			try {
				switch (n.getKind()) {
				case IDENT:
					final Promise<GenType, ResolveError, Void> x = deduceIdentExpression_p(aDeduceTypes2,
							(IdentExpression) n, context);
				...
				case DOT_EXP:
					...
					break;
				case PROCEDURE_CALL:
					final Promise<GenType, ResolveError, Void> x2 = deduceProcedureCall_p((ProcedureCallExpression) n,
							context, aDeduceTypes2);
...
//					x2.fail(); // TODO
				...
				case QIDENT:
					final IExpression expression = Helpers.qualidentToDotExpression2(((Qualident) n));
					@Nullable
					final Promise<GenType, ResolveError, Void> x3 = deduceExpression_p(aDeduceTypes2, expression,
							context);
...
//					x3.fail(); // TODO
	...
	static class __deduceProcedureCall_p {
		...
		public void do_deduceProcedureCall(final ProcedureCallExpression pce, final Context ctx,
	...
	static class __deduceIdentExpression_p {
		...
		@Nullable
		private static GenType do_deduceIdentExpression__VAR(final DeduceTypes2 aDeduceTypes2, final Context ctx,
				final @Nullable GenType R, final @Nullable VariableStatement vs) throws ResolveError {
			final GenType[] result = new GenType[1];
			final ResolveError[] e = { null };

			if (!vs.typeName().isNull()) {
				@Nullable
				final OS_Module lets_hope_we_dont_need_this = null;
				@NotNull
				final Promise<GenType, ResolveError, Void> x = aDeduceTypes2.resolve_type_p(lets_hope_we_dont_need_this,
						new OS_UserType(vs.typeName()), ctx);
...
//				return deduceExpression(vs.initialValue(), ctx); // infinite recursion
				...
				final Promise<GenType, ResolveError, Void> x2 = deduceExpression_p(aDeduceTypes2, vs.initialValue(),
						vs.getContext());
		...
		@NotNull
		private static GenType do_deduceIdentExpression__FUNCTION(final @NotNull GenType R,
				final @Nullable FunctionDef functionDef) {
			R.setResolved(functionDef.getOS_Type());
			return R;
		}

		@NotNull
		private static GenType do_deduceIdentExpression__fali(final DeduceTypes2 aDeduceTypes2, final Context ctx,
				final @NotNull GenType R, final @Nullable FormalArgListItem fali) throws ResolveError {
			GenType result = null;
			...
			if (!fali.typeName().isNull()) {
				...
				final OS_Module lets_hope_we_dont_need_this = null;
				@NotNull
				final GenType ty = aDeduceTypes2.resolve_type(lets_hope_we_dont_need_this,
						new OS_UserType(fali.typeName()), ctx);
				result = ty;
				if (result == null) {
		...
		public void do_deduceIdentExpression(final DeduceTypes2 aDeduceTypes2, final IdentExpression ident,
				final Context ctx) {
			try {
				@Nullable
				GenType result = null;
