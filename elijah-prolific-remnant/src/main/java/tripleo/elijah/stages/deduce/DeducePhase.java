...
//		assert matching_class_list.size() == generatedClasses.size();
	...
	public @NotNull Promise<ClassDefinition, Diagnostic, Void> generateClass(final GenerateFunctions gf,
		...
		@Nullable
		final WlGenerateClass gen = new WlGenerateClass(gf, ci, generatedClasses, codeRegistrar);
		...
		final GeneratedNode lgci : lgc) {
			...
			if (lgci instanceof GeneratedClass) {
				final @NotNull Collection<GeneratedFunction> generatedFunctions = ((GeneratedClass) lgci).functionMap
						.values();
				for (@NotNull
				final GeneratedFunction generatedFunction : generatedFunctions) {
	...
	public void finish(@NotNull final GeneratedClasses lgc22) {
		...
		for (final GeneratedNode generatedNode : lgc22) {
			if (generatedNode instanceof final @NotNull GeneratedClass generatedClass) {
				@NotNull
				final Collection<GeneratedFunction> functions = generatedClass.functionMap.values();
				for (@NotNull
				final GeneratedFunction generatedFunction : functions) {
				...
				@NotNull
				final Collection<GeneratedFunction> functions = generatedNamespace.functionMap.values();
				for (@NotNull
				final GeneratedFunction generatedFunction : functions) {
			...
			for (final GeneratedNode generatedNode : lgc22) {
				if (generatedNode instanceof final @NotNull GeneratedClass generatedClass) {
					if (generatedClass.getKlass() == classStatement) {
						...
						@NotNull
						final Collection<GeneratedClass> cmv = generatedClass.classMap.values();
						for (@NotNull
						final GeneratedClass aClass : cmv) {
		 ...
		 */
		for (@NotNull
		final FoundElement foundElement : foundElements) {
		...
		for (final GeneratedNode generatedNode : lgc22) {
			if (generatedNode instanceof final @NotNull GeneratedContainer generatedContainer) {
				...
				for (@NotNull
				final ResolvedVariables resolvedVariables : x) {
		...
		@NotNull
		final List<GeneratedClass> gcs = new ArrayList<GeneratedClass>();
...
//								}
		...
		for (final Map.@NotNull Entry<FunctionDef, Collection<GeneratedFunction>> entry : functionMap.asMap()
				.entrySet()) {
			for (@NotNull
			final FunctionMapHook functionMapHook : functionMapHooks) {
	...
	public NamespaceInvocation registerNamespaceInvocation(final NamespaceStatement aNamespaceStatement) {
		...
		@NotNull
		final NamespaceInvocation nsi = new NamespaceInvocation(aNamespaceStatement);
...
//				sanityChecks(generatedNamespace.constructors.values());
	...
	private void sanityChecks(@NotNull final Collection<GeneratedFunction> aGeneratedFunctions) {
		for (@NotNull
		final GeneratedFunction generatedFunction : aGeneratedFunctions) {
			for (@NotNull
			final IdentTableEntry identTableEntry : generatedFunction.idte_list) {
				...
				for (@NotNull
				final TypeTableEntry pot_tte : identTableEntry.potentialTypes()) {
