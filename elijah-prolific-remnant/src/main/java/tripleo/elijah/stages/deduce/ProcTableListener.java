...
public class ProcTableListener implements BaseTableEntry.StatusListener {
	...
	public ProcTableListener(final ProcTableEntry pte, final BaseGeneratedFunction generatedFunction,
			final DeduceTypes2.@NotNull DeduceClient2 dc) {
		this.pte = pte;
...
//		pte.setResolvedElement(e); // README already done
		if (e instanceof ClassStatement) {
			...
			fi = dc.newFunctionInvocation(ConstructorDef.defaultVirtualCtor, pte, ci); // TODO might not be virtual
																						// ctor, so check
	...
	private void resolved_element_pte_FunctionDef(final Constructable co, @NotNull final ProcTableEntry pte,
			final AbstractDependencyTracker depTracker, @NotNull final FunctionDef fd) {
		@Nullable
		final FunctionInvocation fi;
		...
		if (pte.expression_num != null) {
			...
			if (dp.size() > 1) {
				@Nullable
				final OS_Element el_self = dp.getElement(dp.size() - 2);
				...
				if (parent instanceof IdentExpression) {
					...
					resolved_element_pte_FunctionDef_FormalArgListItem(co, pte, depTracker, fd,
							(FormalArgListItem) parent);
				} else if (parent instanceof VariableStatement) {
					resolved_element_pte_FunctionDef_VariableStatement(co, pte, depTracker, fd,
							(VariableStatement) parent);
				} else {
					@NotNull
					final E_Is_FunctionDef e_Is_FunctionDef = new E_Is_FunctionDef(pte, fd, parent).invoke(null);
	...
	private void resolved_element_pte_FunctionDef_IdentExpression(final Constructable co, final ProcTableEntry pte,
			final AbstractDependencyTracker depTracker, @NotNull final FunctionDef fd,
			@NotNull final IdentExpression parent) {
		@Nullable
		final InstructionArgument vte_ia = generatedFunction.vte_lookup(parent.getText());
		assert vte_ia != null;
	...
	private void resolved_element_pte_FunctionDef_FormalArgListItem(final Constructable co, final ProcTableEntry pte,
			final AbstractDependencyTracker depTracker, @NotNull final FunctionDef fd, final FormalArgListItem parent) {
		final FormalArgListItem fali = parent;
		@Nullable
		final InstructionArgument vte_ia = generatedFunction.vte_lookup(fali.name());
		assert vte_ia != null;
		final @NotNull VariableTableEntry variableTableEntry = ((IntegerIA) vte_ia).getEntry();
		VTE_TypePromises.resolved_element_pte(co, pte, depTracker, fd, variableTableEntry, this);
	}

	private void resolved_element_pte_FunctionDef_VariableStatement(final Constructable aCo, final ProcTableEntry aPte,
			final AbstractDependencyTracker aDepTracker, final FunctionDef aFd, final VariableStatement aParent) {
...
//		resolved_element_pte_FunctionDef_VariableStatement(aCo, aPte, aDepTracker, aPte, aFd, aParent);
	...
	void finish(@Nullable final Constructable co, @Nullable final AbstractDependencyTracker depTracker,
			@NotNull final FunctionInvocation aFi, @Nullable final GenType aGenType) {
	...
	private void resolved_element_pte_FunctionDef_dunder(final Constructable co,
			final AbstractDependencyTracker depTracker, @NotNull final ProcTableEntry pte,
			@NotNull final FunctionDef fd, OS_Element parent) {
		@Nullable
		final FunctionInvocation fi;
		final GenType genType;
		if (parent instanceof IdentExpression) {
			@Nullable
			final InstructionArgument vte_ia = generatedFunction.vte_lookup(((IdentExpression) parent).getText());
			assert vte_ia != null;
			final @NotNull VariableTableEntry variableTableEntry = ((IntegerIA) vte_ia).getEntry();
			VTE_TypePromises.resolved_element_pte(co, pte, depTracker, fd, variableTableEntry, this);
		} else {
			...
			if (fd == parent) {
				...
				if (x.getAttached() == null && x.tableEntry == null) {
					...
					@Nullable
					final InstructionArgument vte_ia = generatedFunction.vte_lookup(text);
					if (vte_ia != null) {
			...
			@NotNull
			final E_Is_FunctionDef e_Is_FunctionDef = new E_Is_FunctionDef(pte, fd, parent).invoke(typeName);
			fi = e_Is_FunctionDef.getFi();
			genType = e_Is_FunctionDef.getGenType();
			finish(co, depTracker, fi, genType);
		}
	...
	private void resolved_element_pte_FunctionDef_VariableStatement(final Constructable co,
		...
		if (ia != null) {
			if (ia instanceof IdentIA) {
				@NotNull
				final IdentTableEntry identTableEntry = ((IdentIA) ia).getEntry();
				final int y = 2;
		...
		@Nullable
		final InstructionArgument vte_ia = generatedFunction.vte_lookup(variableStatement.getName());
//		assert vte_ia != null;
		if (vte_ia == null) {
			return;
