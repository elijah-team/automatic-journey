...
public class ChooseHashDirectoryNameBehavior implements ChooseDirectoryNameBehavior {
	...
	private @NotNull File choose_dir_name() {
		...
		final List<String> recordedread_filenames = recordedreads.stream().map(File::toString)
...
//				.forEachOrdered(x -> sortedMap.put(x.getKey(), x.getValue()));
		...
		recordedread_filenames.stream().sorted().map(digestUtils::digestAsHex).forEach(sha256 -> sb1.append(sha256));
