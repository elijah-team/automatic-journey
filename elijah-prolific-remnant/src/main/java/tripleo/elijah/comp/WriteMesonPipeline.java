...
//	private final GenerateResult gr;
	...
	private void write_root(@NotNull final Multimap<CompilerInstructions, String> lsp_outputs,
			final List<String> aDep_dirs) throws IOException {
		final CharSink root_file = c.getIO().openWrite(getPath("meson.build"));
		try {
			final String project_name = c.getProjectName();
			final String project_string = String
...
//			root_file.accept("\n");
			...
			final String deps_names = String_join(", ", aDep_dirs.stream().map(x -> String.format("%s", x)) // TODO _lib
																											// ??
					.collect(Collectors.toList()));
			root_file.accept(String.format("%s_bin = executable('%s', link_with: [ %s ], install: true)", project_name,
					project_name, deps_names)); // dependencies, include_directories
		} finally {
	...
	private void write_lsp(@NotNull final Multimap<CompilerInstructions, String> lsp_outputs,
			final CompilerInstructions compilerInstructions, final String aSub_dir) throws IOException {
		final Path path = FileSystems.getDefault().getPath("COMP", c.getCompilationNumberString(), aSub_dir,
				"meson.build");
		...
		try {
			...
			final Set<String> files = files_.stream().filter(x -> x.endsWith(".c"))
					.map(x -> String.format("\t'%s',", pullFileName(x))).collect(Collectors.toSet()); // TODO
																										// .toUnmodifiableSet
																										// -- language
																										// level 10
			sub_file.accept(String.format("%s_sources = files(\n%s\n)", aSub_dir, String_join("\n", files)));
			sub_file.accept("\n");
			sub_file.accept(String.format("%s = static_library('%s', %s_sources, install: false,)", aSub_dir, aSub_dir,
					aSub_dir)); // include_directories, dependencies: [],
			sub_file.accept("\n");
			sub_file.accept("\n");
...
//			Collection<String> files_ = lsp_outputs.get(compilerInstructions);
			final List<String> files = List_of(
					"'Prelude.c'")/*
			...
			prel_file.accept("\n");
			prel_file.accept("Prelude = static_library('Prelude', Prelude_sources, install: false,)"); // include_directories,
																										// dependencies:
																										// [],
			prel_file.accept("\n");
			prel_file.accept("\n");
