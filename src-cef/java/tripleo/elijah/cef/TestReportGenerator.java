...
/**
 * Just a small class that assists users in generating test reports. You can
 * ignore this, it is not really part of the example.
 *
		...
		if (EnumPlatform.getCurrentPlatform().getOs().isLinux()) {
			try {
				final String version = (new BufferedReader(
						new InputStreamReader((new ProcessBuilder("lsb_release", "-ds")).start().getInputStream())))
						.readLine();
			...
			try {
				final String name = (new BufferedReader(new InputStreamReader(
						(new ProcessBuilder("sw_vers", "-productName")).start().getInputStream()))).readLine();
				final String version = (new BufferedReader(new InputStreamReader(
						(new ProcessBuilder("sw_vers", "-productVersion")).start().getInputStream()))).readLine();
